...
class VmapTest(tf.test.TestCase, parameterized.TestCase):
    def test_vmap_in_axes_list(self):
        ...
        dictionary = {"a": 5.0, "b": tf_np.ones(2)}
        x = tf_np.zeros(3)
        y = tf_np.arange(3.0)
    ...
    def test_vmap_in_axes_leaf_types(self):
        with self.assertRaisesRegex(
            ...
            extensions.vmap(lambda x: x, in_axes=(tf_np.array([1.0, 2.0]),))(
                tf_np.array([1.0, 2.0])
            )
    ...
    def test_vmap_out_axes_leaf_types(self):
        with self.assertRaisesRegex(
            ...
            extensions.vmap(lambda x: x, out_axes=(tf_np.array([1.0, 2.0]),))(
                tf_np.array([1.0, 2.0])
            )
    ...
    def test_vmap_mismatched_axis_sizes_error_message_issue_705(self):
        ...
        with self.assertRaisesRegex(
            ...
            extensions.vmap(lambda x: x, in_axes=None)(tf_np.array([1.0, 2.0]))
        ...
        with self.assertRaisesRegex(
            ...
            extensions.vmap(lambda x: x, in_axes=0, out_axes=(2, 3))(
                tf_np.array([1.0, 2.0])
            )
    ...
    def test_negative_axes(self):
        ...
        self.assertAllClose(
            x.transpose(1, 0, 2), extensions.vmap(identity, in_axes=0, out_axes=-2)(x)
        )
        self.assertAllClose(
            x.transpose(1, 2, 0), extensions.vmap(identity, in_axes=0, out_axes=-1)(x)
        )

        self.assertAllClose(
            np.full((5,), 7),
            extensions.vmap(lambda *xs: xs, in_axes=(0, None), out_axes=(0, -1))(
                np.arange(5), 7
            )[1],
        )
