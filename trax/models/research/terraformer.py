...
def ConfigurableTerraformer(
    ...
    # Vector embeddings.
    in_encoder, out_encoder, output_vocab_size = ct.EmbeddingAndPositionalEncodings(
        input_vocab_size,
        d_model1,
        mode,
        dropout,
    ...
    if mode == "predict":
        ...
        tl.rnn.ScanSRUCell = functools.partial(
            tl.rnn.ScanSRUCell, monkey_patched_mask=portal_mask
        )
        ...
        if input_vocab_size is not None:
            return tl.AssertFunction(
                ...
                tl.Serial(  # tok_e tok_d
                    tl.Select([0, 0, 0, 1]),
                    tl.Parallel(in_encoder, [tl.PaddingMask(), _RemoveAxes12()]),
                ),
            ...
            return tl.AssertFunction(
                ...
                tl.Serial(  # vec_e mask_e tok_d
                    tl.Select([0, 1, 1, 2]), tl.Parallel(in_encoder, [], _AsTokenIDs())
                ),
            )  # vec_e mask_e tok_e tok_d
    ...
    return tl.Serial(
        ...
        tl.Parallel(tl.Select([0]), portal_mask),
        ...
        # Run decoder blocks.
        _ReversibleSerialForget(
            decoder_blocks, d_model2, n_layers_forget, forget_dense
        ),  # vec_ed1 vec_ed2 tok_e tok_d
...
def _InsertAxes12():
    ...
    return tl.Fn(
        "InsertAxes12", lambda x: jnp.reshape(x, (x.shape[0], 1, 1, x.shape[1]))
    )
...
def _ConvertToNaNsOnAnyZero():
    def _convert_to_nans(x, y):
        ...
        return jnp.where(jnp.all(y, keepdims=False), x, x / 0.0), y
