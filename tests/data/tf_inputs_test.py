...
class TFInputsTest(tf.test.TestCase):
    ...
    def test_TFDS_mnli_split_is_eval(self):
        with mock.patch("tensorflow_datasets.load") as tfds_load:
            with mock.patch(
                "trax.data.tf_inputs.download_and_prepare", lambda _, data_dir: data_dir
            ):
                _ = tf_inputs.TFDS(
                    "glue/mnli", keys=("premise", "hypothesis"), train=False
                )
    ...
    def test_tokenize_keys_reservedids(self):
        ...
        tok_char2 = list(
            tf_inputs.tokenize(dataset(), keys=[0], vocab_type="char", n_reserved_ids=2)
        )
    ...
    def test_tokenize_dict(self):
        ...
        tok_char2 = list(tf_inputs.tokenize(dataset(), keys=["a"], vocab_type="char"))
    ...
    def test_process_single_mathqa_example_0(self):
        ...
        (
            ...
            list_op,
            list_num,
        ) = tf_inputs.process_single_mathqa_example(example)
    ...
    def test_process_single_mathqa_example_with_import(self):
        ...
        self.assertAllClose(
            tf_inputs.execute_mathqa_dsl_program(problem, [example["linear_formula"]]),
            14,
        )
