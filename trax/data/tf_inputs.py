...
def _train_and_eval_dataset_v1(
    problem_name, data_dir, train_shuffle_files, eval_shuffle_files
):
    """Return train and evaluation datasets, feature info and supervised keys."""
...
def _get_vocab(vocab_type="subword", vocab_file=None, vocab_dir=None, extra_ids=0):
    ...
    return t5_data().SentencePieceVocabulary(
        sentencepiece_model_file=path, extra_ids=extra_ids
    )
...
@gin.configurable(module="trax.data")
def SentencePieceTokenize(spm_path=None, extra_ids=0):  # pylint: disable=invalid-name
    """Returns a function that maps text to integer arrays."""
    return lambda g: sentencepiece_tokenize(  # pylint: disable=g-long-lambda
        g, spm_path=spm_path, extra_ids=extra_ids
    )
...
def c4_bare_preprocess_fn(
    dataset, training=True, spm_path=None, copy_pretokenized=True, sequence_length=None
):
    ...
    dataset = dataset.map(
        encode_string_features_fn, num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
...
def CorpusToRandomChunks(
    ...
    return TFDS(
        dataset_name,
        tfds_preprocess_fn=functools.partial(
            t5_data().preprocessors.random_split_text, max_words_per_segment=num_tokens
        ),
        train=train,
        keys=["text"],
    )
