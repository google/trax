...
def _FeedForward(d_model, d_ff, dropout, activation, act_dropout, use_bfloat16, mode):
...
def FeedForwardWithOptions(
    ...
    if ff_sparsity and ff_sparsity_type == "1inN":
        ...
        if isinstance(ff_sparsity, str):
            ...
            ff_sparsity = [
                (float(x) if "." in x else int(x)) for x in ff_sparsity.split()
            ]
        ...
        ff = _FeedForward(
            d_model, d_ff, dropout, ff_activation, ff_dropout, use_bfloat16, mode
        )
    ...
    if ff_use_sru:
        ...
        block = (
            [tl.LayerNorm(center=center_layernorm), tl.Dense(sru_n_units)]
            + sru
            + [tl.Dense(d_model)]
        )
...
def PositionalEncoder(
    ...
    if not pos_type:
        positional_encoding = tl.PositionalEncoding(
            ...
            max_offset_to_add=pos_max_offset_to_add,
            mode=mode,
        )
...
def EmbeddingAndPositionalEncodings(
    input_vocab_size,
    d_model,
    mode,
    embedding_dropout,
...
def ConfigurableTransformerEncoder(
    vocab_size,
    n_classes=10,
    d_model=512,
    d_ff=2048,
...
def ConfigurableTransformer(
    ...
    """
    in_encoder, out_encoder, output_vocab_size = EmbeddingAndPositionalEncodings(
        input_vocab_size,
        d_model,
        mode,
        dropout,
    ...
    return tl.Serial(
        ...
        # Encode.
        tl.Branch(tl.Select([0]), tl.PaddingMask()),  # tok_e masks ..... .....
        encoder,  # vec_e ..... ..... .....
        # Decode.
        tl.Select([2, 1, 0]),  # tok_d masks vec_e .....
        tl.ShiftRight(mode=mode),  # tok_d ..... ..... .....
        out_encoder,  # vec_d ..... ..... .....
        tl.Branch(tl.Select([0]), tl.EncoderDecoderMask()),  # vec_d masks ..... .....
