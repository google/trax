...
class PolicyTrainTask(training.TrainTask):
    ...
    def __init__(
        ...
        Args:
          trajectory_batch_stream: Generator of trax.reinforcement.task.TimeStepBatch.
          ...
          advantage_estimator: Function
            ...
            functions from trax.reinforcement.advantages.
        ...
        sample_batch = self.policy_batch(next(trajectory_batch_stream), shape_only=True)
    ...
    def policy_batch(self, trajectory_batch, shape_only=False):
        ...
        Args:
          trajectory_batch: trax.reinforcement.task.TimeStepBatch with a batch of trajectory
        ...
        """
        advantages = self.calculate_advantages(trajectory_batch, shape_only=shape_only)
...
class PolicyEvalTask(training.EvalTask):
    ...
    def __init__(self, train_task, n_eval_batches=1, head_selector=()):
        ...
        metrics = {
            name: tl.Serial(head_selector, metric) for (name, metric) in metrics.items()
        }
    ...
    def advantage_metrics(self):
        ...
        return {
            "advantage_" + name: make_metric(fn)
            for (name, fn) in [
                ("mean", jnp.mean),
                ("std", jnp.std),
            ]
