...
JAX_ONE_TO_ONE_OP_RECORDS = [
    ...
    op_record(
        ...
        inexact_dtypes,
        all_shapes,
        partial(jtu.rand_default, scale=1),
        ["rev"],
        tolerance={
    ...
    op_record(
        "greater",
        2,
        minus(all_dtypes, complex_dtypes),
        all_shapes,
        jtu.rand_some_equal,
        [],
    ),
    ...
    op_record(
        "log", 1, number_dtypes, all_shapes, jtu.rand_positive, ["rev"], inexact=True
    ),
    ...
    op_record(
        "nextafter",
        2,
        [f for f in float_dtypes if f not in (lnp.bfloat16, onp.float16)],
        all_shapes,
        jtu.rand_default,
        ["rev"],
        inexact=True,
    ...
    op_record(
        "array_equal", 2, number_dtypes, all_shapes, jtu.rand_some_equal, ["rev"]
    ),
    ...
    op_record(
        "signbit",
        1,
        default_dtypes + bool_dtypes,
        all_shapes,
        jtu.rand_some_inf_and_nan,
        ["rev"],
    ),
    op_record(
        "sin", 1, number_dtypes, all_shapes, jtu.rand_default, ["rev"], inexact=True
    ),
    op_record(
    ...
    op_record(
        ...
        number_dtypes,
        all_shapes,
        partial(jtu.rand_uniform, -1.5, 1.5),
        ["rev"],
    ...
    op_record(
        "sinh",
        1,
        minus(number_dtypes, [onp.float16]),
        all_shapes,
        jtu.rand_default,
        ["rev"],
        inexact=True,
    ...
    op_record(
        "arcsin",
        1,
        minus(float_dtypes, [onp.float16]),
        all_shapes,
        jtu.rand_small,
        ["rev"],
        inexact=True,
...
JAX_COMPOUND_OP_RECORDS = [
    ...
    op_record(
        "cbrt", 1, default_dtypes, all_shapes, jtu.rand_default, ["rev"], inexact=True
    ),
    ...
    op_record(
        "divmod",
        2,
        minus(int_dtypes + float_dtypes, [onp.float16]),
        all_shapes,
        jtu.rand_nonzero,
        [],
    ),
    ...
    op_record(
        "floor_divide",
        2,
        minus(number_dtypes, complex_dtypes),
        all_shapes,
        jtu.rand_nonzero,
        ["rev"],
    ),
    op_record(
        "heaviside", 2, default_dtypes, all_shapes, jtu.rand_default, [], inexact=True
    ),
    op_record(
        "hypot", 2, default_dtypes, all_shapes, jtu.rand_default, [], inexact=True
    ),
    op_record(
        "kron",
        2,
    ...
    op_record(
        "isfinite",
        1,
        minus(inexact_dtypes, complex_dtypes),
        all_shapes,
        jtu.rand_some_inf_and_nan,
        [],
    ),
    ...
    op_record(
        "log2", 1, number_dtypes, all_shapes, jtu.rand_positive, ["rev"], inexact=True
    ),
    op_record(
        "log10", 1, number_dtypes, all_shapes, jtu.rand_positive, ["rev"], inexact=True
    ),
    op_record(
        "log1p",
        1,
    ...
    op_record(
        "remainder",
        2,
        minus(default_dtypes, [onp.float16]),
        all_shapes,
        jtu.rand_nonzero,
        [],
        tolerance={onp.float16: 1e-2},
    ...
    op_record(
        "mod", 2, minus(default_dtypes, [onp.float16]), all_shapes, jtu.rand_nonzero, []
    ),
    op_record(
        "sinc",
        1,
        [t for t in number_dtypes if t != lnp.bfloat16],
        all_shapes,
        jtu.rand_default,
        ["rev"],
...
JAX_BITWISE_OP_RECORDS = [
    op_record(
        "bitwise_and", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_default, []
    ),
    op_record(
        "bitwise_not", 1, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_default, []
    ),
    op_record(
        "bitwise_or", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_default, []
    ),
    op_record(
        "bitwise_xor", 2, int_dtypes + unsigned_dtypes, all_shapes, jtu.rand_default, []
    ),
]
...
JAX_OPERATOR_OVERLOADS = [
    ...
    op_record(
        "__truediv__", 2, number_dtypes, all_shapes, jtu.rand_nonzero, [], inexact=True
    ),
...
JAX_RIGHT_OPERATOR_OVERLOADS = [
    ...
    op_record(
        "__rtruediv__", 2, number_dtypes, all_shapes, jtu.rand_nonzero, [], inexact=True
    ),
...
if numpy_version >= (1, 15):
    JAX_COMPOUND_OP_RECORDS += [
        op_record(
            "isclose",
            2,
            [t for t in all_dtypes if t != lnp.bfloat16],
            all_shapes,
            jtu.rand_small_positive,
            [],
        ),
...
class LaxBackedNumpyTests(jtu.TestCase):
    ...
    def _GetArgsMaker(self, rng, shapes, dtypes, onp_arrays=True):
        def f():
            out = [
                rng(shape, dtype or lnp.float_) for shape, dtype in zip(shapes, dtypes)
            ]
            ...
            jtu.cases_from_list(
                {
                    ...
                    "shapes": shapes,
                    "dtypes": dtypes,
                    "onp_op": getattr(onp, rec.name),
                    "lnp_op": getattr(lnp, rec.name),
    ...
    def testOp(
        ...
        self._CheckAgainstNumpy(
            _promote_like_lnp(onp_op, inexact),
            lnp_op,
            args_maker,
        ...
        check_xla = not (
            lnp_op.__name__ == "power"
            and set(dtypes).intersection((onp.int32, onp.int64))
        )
    ...
    def testOperatorOverload(self, name, rng_factory, shapes, dtypes, tol):
        ...
        self._CompileAndCheck(
            fun,
            args_maker,
            check_dtypes=True,  # not scalar_arg and not empty_shape,
            atol=tol,
            rtol=tol,
        )
    ...
    def testRightOperatorOverload(
        self, name, rng_factory, shapes, dtypes, op_tolerance
    ):
            ...
            jtu.cases_from_list(
                {
                    "testcase_name": "{}_inshape={}_axis={}_dtype={}_keepdims={}".format(
                        ...
                        axis,
                        "None" if out_dtype is None else onp.dtype(out_dtype).name,
                        keepdims,
                    ),
                    "rng_factory": rec.rng_factory,
                    "shape": shape,
    ...
    def testReducer(
        ...
        self._CompileAndCheck(
            lnp_fun, args_maker, check_dtypes=True, atol=tol, rtol=tol
        )
    ...
    def testNonzero(self, shape, dtype):
        ...
        self._CompileAndCheck(
            ...
            check_experimental_compile=False,
    ...
    def testCross(self, lhs_shape, lhs_dtype, rhs_shape, rhs_dtype, axes, rng_factory):
        ...
        tol = max(
            jtu.tolerance(lhs_dtype, tol_spec), jtu.tolerance(rhs_dtype, tol_spec)
        )
        self._CheckAgainstNumpy(
    ...
    def testDot(self, lhs_shape, lhs_dtype, rhs_shape, rhs_dtype, rng_factory):
        ...
        self._CheckAgainstNumpy(
            onp_dot, lnp.dot, args_maker, check_dtypes=True, tol=tol
        )
    ...
    def testClipStaticBounds(self, shape, dtype, a_min, a_max, rng_factory):
        ...
        self._CompileAndCheck(
            lnp_fun,
            args_maker,
            check_dtypes=not is_x32_scalar,
            atol=tol,
            rtol=tol,
            check_incomplete_shape=True,
        )
    ...
    def testOperatorRound(self):
        self.assertAllClose(
            round(onp.float32(7.532), 1), round(lnp.float32(7.5), 1), check_dtypes=True
        )
        self.assertAllClose(
            round(onp.float32(1.234), 2),
            round(lnp.float32(1.234), 2),
            check_dtypes=True,
        )
        self.assertAllClose(
            round(onp.float32(1.234)), round(lnp.float32(1.234)), check_dtypes=False
        )
        self.assertAllClose(
            round(onp.float32(7.532), 1),
            round(lnp.array(7.5, lnp.float32), 1),
            check_dtypes=True,
        )
        self.assertAllClose(
            round(onp.float32(1.234), 2),
            round(lnp.array(1.234, lnp.float32), 2),
            check_dtypes=True,
        )
        self.assertAllClose(
            round(onp.float32(1.234)),
        ...
        jtu.cases_from_list(
            {
                "testcase_name": "_axis={}_baseshape=[{}]_dtypes=[{}]".format(
                    axis,
                    ",".join(str(d) for d in base_shape),
    ...
    def testConcatenate(self, axis, base_shape, arg_dtypes, rng_factory):
        ...
        shapes = [
            base_shape[:wrapped_axis] + (size,) + base_shape[wrapped_axis + 1 :]
    ...
    def testAppend(self, axis, base_shape, arg_dtypes, rng_factory):
        ...
        def onp_fun(arr, values):
            ...
            values = (
                values.astype(onp.float32) if lnp.bfloat16 == values.dtype else values
            )
    ...
    def testDiagIndices(self, ndim, n):
        onp.testing.assert_equal(onp.diag_indices(n, ndim), lnp.diag_indices(n, ndim))
    ...
    @named_parameters(
        jtu.cases_from_list(
            ...
            for axis2 in [
                a
                for a in range(-len(shape), len(shape))
    ...
    @named_parameters(
        jtu.cases_from_list(
            ...
            for shape in [
                ...
                (2,),
                (3, 0),
                onp.array((4, 5, 6), dtype=onp.int32),
        ...
        jtu.cases_from_list(
            {
                "testcase_name": "_shape={}_axis={}_weights={}_returned={}".format(
                    ...
                    (
                        ...
                        else jtu.format_shape_dtype_string(weights_shape, dtype)
                    ),
                    returned,
                ),
                "rng_factory": jtu.rand_default,
        ...
        jtu.cases_from_list(
            ...
            for i, (arg, dtype) in enumerate(
                [
                    ...
                    (3.0, lnp.float_),
                    ([1, 2, 3], lnp.int_),
                    ([1.0, 2.0, 3.0], lnp.float_),
                    ([[1, 2], [3, 4], [5, 6]], lnp.int_),
                    ([[1, 2.0], [3, 4], [5, 6]], lnp.float_),
                    ([[1.0, 2j], [3.0, 4.0], [5.0, 6.0]], lnp.complex_),
                    (
                        [
                            [3, onp.array(2, dtype=lnp.float_), 1],
                            onp.arange(3.0, dtype=lnp.float_),
                        ],
                        lnp.float_,
                    ),
                ]
    ...
    def testMemoryView(self):
        ...
        self.assertAllClose(ans, onp.array([0x2A], dtype=onp.uint8), check_dtypes=True)
    ...
    def testOnpMean(self):
        ...
        x = lnp.add(lnp.eye(3, dtype=lnp.float_), 0.0)
        ans = onp.mean(x)
        ...
        jtu.cases_from_list(
            {
                "testcase_name": "_{}_shifts={}_axis={}".format(
                    jtu.format_shape_dtype_string(shape, dtype), shifts, axis
                ),
                "rng_factory": rng_factory,
                "shape": shape,
                "dtype": dtype,
        ...
        jtu.cases_from_list(
            {
                ...
                "rng_factory": rng_factory,
                "rng_indices_factory": rng_indices_factory,
                "shape": shape,
        ...
        jtu.cases_from_list(
            {
                "testcase_name": "_shape={}_n={}_increasing={}".format(
                    jtu.format_shape_dtype_string([shape], dtype), n, increasing
                ),
                "dtype": dtype,
                "shape": shape,
                "n": n,
    ...
    def testVander(self, shape, dtype, n, increasing, rng_factory):
        ...
        self._CheckAgainstNumpy(
            onp_fun, lnp_fun, args_maker, check_dtypes=False, tol={onp.float32: 1e-3}
        )
        self._CompileAndCheck(
            lnp_fun,
            args_maker,
    ...
    def testNanToNum(self, rng_factory, shape, dtype):
        ...
        self._CompileAndCheck(lnp.nan_to_num, args_maker, check_dtypes=check_dtypes)
    ...
    @named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_op={}_a_shape={}_q_shape={}_axis={}_keepdims={}".format(
                    op,
            ...
            for (op, q_rng) in (
                ("percentile", jtu.rand_uniform(low=0.0, high=100.0)),
                ("quantile", jtu.rand_uniform(low=0.0, high=1.0)),
                ("median", jtu.rand_uniform(low=0.0, high=1.0)),
            )
    ...
    @jtu.disable
    def testQuantile(
        self, op, a_rng, q_rng, a_shape, a_dtype, q_shape, q_dtype, axis, keepdims
    ):
        ...
        def onp_fun(*args):
            args = [
                x if lnp.result_type(x) != lnp.bfloat16 else onp.asarray(x, onp.float32)
                for x in args
            ]
    ...
    @named_parameters(
        jtu.cases_from_list(
            ...
            for shapes in filter(
                _shapes_are_broadcast_compatible, CombosWithReplacement(all_shapes, 3)
            )
    ...
    def testWhereScalarPromotion(self):
        x = lnp.where(lnp.array([True, False]), 3, lnp.ones((2,), dtype=lnp.float32))
    ...
    def testSelect(self, rng_factory, shapes, dtypes):
        ...
        def args_maker():
            ...
            choicelist = [
                rng(shape, dtype) for shape, dtype in zip(shapes[n:-1], dtypes[:n])
            ]
        ...
        def onp_fun(condlist, choicelist, default):
            choicelist = [
                x if lnp.bfloat16 != lnp.result_type(x) else x.astype(onp.float32)
                for x in choicelist
            ]
        ...
        self._CheckAgainstNumpy(onp_fun, lnp.select, args_maker, check_dtypes=False)
        self._CompileAndCheck(
            lnp.select,
            args_maker,
            check_dtypes=True,
    ...
    def testArange(self):
        ...
        self.assertAllClose(
            lnp.arange(77), onp.arange(77, dtype=lnp.int_), check_dtypes=True
        )
        self.assertAllClose(
            lnp.arange(2, 13), onp.arange(2, 13, dtype=lnp.int_), check_dtypes=True
        )
        self.assertAllClose(
            lnp.arange(4, 21, 9),
            onp.arange(4, 21, 9, dtype=lnp.int_),
            check_dtypes=True,
        )
        self.assertAllClose(
            lnp.arange(53, 5, -3),
        ...
        self.assertAllClose(
            lnp.arange(77, dtype=float), onp.arange(77, dtype=float), check_dtypes=True
        )
        self.assertAllClose(
    ...
    def testIssue764(self):
        ...
        expected = onp.array(
            [3.71669453e-165, 4.72999108e-168, 6.01954653e-171, 7.66067839e-174],
            onp.float64,
        )
    ...
    def testIssue777(self):
        ...
        self.assertAllClose(
            f(x), onp.array([0.0, 0.0, 0.0, 0.25], dtype=onp.float32), check_dtypes=True
        )
    ...
    def testMathSpecialFloatValues(self, op, dtype):
        ...
        for x in (
            ...
            onp.inf,
            lnp.finfo(dtype).max,
            onp.sqrt(lnp.finfo(dtype).max),
            onp.sqrt(lnp.finfo(dtype).max) * 2.0,
        ):
            if (
                op in ("sin", "cos", "tan", "arctan")
    ...
    def testReductionOfOutOfBoundsAxis(self):  # Issue 888
        x = lnp.ones((3, 4))
        self.assertRaises(tf.errors.InvalidArgumentError, lambda: lnp.sum(x, axis=2))
    ...
    @named_parameters(
        jtu.cases_from_list(
            {
                "testcase_name": "_shape={}_dtype={}_out_dtype={}_axis={}_ddof={}_keepdims={}".format(
                    shape, dtype, out_dtype, axis, ddof, keepdims
                ),
                "shape": shape,
                "dtype": dtype,
                "out_dtype": out_dtype,
    ...
    def testVar(self, shape, dtype, out_dtype, axis, ddof, keepdims, rng_factory):
        ...
        def onp_fun(x):
            out = onp.var(
                x.astype(lnp.promote_types(onp.float32, dtype)),
                axis=axis,
                ddof=ddof,
                keepdims=keepdims,
            )
        ...
        lnp_fun = partial(
            lnp.var, dtype=out_dtype, axis=axis, ddof=ddof, keepdims=keepdims
        )
        tol = jtu.tolerance(
            out_dtype,
            {
    ...
    def testMeshGrid(self, shapes, dtype, indexing, sparse, rng_factory):
        ...
        args_maker = self._GetArgsMaker(
            rng, [(x,) for x in shapes], [dtype] * len(shapes)
        )
    ...
    def testLinspace(
        self, start_shape, stop_shape, num, endpoint, retstep, dtype, rng_factory
    ):
    ...
    def testLogspace(
        self, start_shape, stop_shape, num, endpoint, base, dtype, rng_factory
    ):
        if (
            dtype in int_dtypes
            and jtu.device_under_test() in ("gpu", "tpu")
            and not FLAGS.enable_x64
        ):
            raise unittest.SkipTest(
                "GPUx32 truncated exponentiation"
    ...
    def testGeomspace(self, start_shape, stop_shape, num, endpoint, dtype, rng_factory):
        ...
        def args_maker():
            ...
            start, stop = self._GetArgsMaker(
                rng, [start_shape, stop_shape], [dtype, dtype]
            )()
    ...
    def testDisableNumpyRankPromotionBroadcasting(self):
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                msg = str(w[-1].message)
                expected_msg = (
                    "Following NumPy automatic rank promotion for add on "
                    "shapes (2,) (1, 2)."
                )
    ...
    def testReluGradientConstants(self):
        ...
        self.assertFalse(
            any(
                onp.array_equal(x, onp.full((3, 4), 2.0, dtype=onp.float32))
                for x in consts
            )
        )
    ...
    def testBroadcastToIntIssue1548(self):
        self.assertAllClose(
            lnp.broadcast_to(1, (3, 2)), onp.ones((3, 2)), check_dtypes=False
        )
    ...
    def testPrecision(self):
        ...
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.dot, precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.dot, precision=HIGHEST), ones_3d, ones_3d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.matmul, precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.vdot, precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.tensordot, axes=2, precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST,
        ...
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.einsum, "i,i", precision=HIGHEST), ones_1d, ones_1d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.einsum, "ij,ij", precision=HIGHEST), ones_2d, ones_2d
        )
        jtu.assert_dot_precision(
            HIGHEST, partial(lnp.inner, precision=HIGHEST), ones_1d, ones_1d
        )
        ...
        jtu.cases_from_list(
            {
                "testcase_name": "_{}_{}_{}_{}".format(
                    shape,
                    jtu.dtype_str(key_dtype),
                    jtu.dtype_str(value_dtype),
                    dimension,
                ).replace(" ", ""),
                "shape": shape,
                "key_dtype": key_dtype,
...
GRAD_SPECIAL_VALUE_TEST_RECORDS = [
    GradSpecialValuesTestSpec(lnp.arcsinh, [0.0, 1000.0], 2),
    GradSpecialValuesTestSpec(lnp.arccosh, [1000.0], 2),
    GradSpecialValuesTestSpec(lnp.arctanh, [0.0], 2),
...
class NumpyGradTests(jtu.TestCase):
    ...
    def testOpGradSpecialValue(self, op, special_value, order):
        check_grads(
            op, (special_value,), order, ["fwd", "rev"], atol={onp.float32: 3e-3}
        )
    ...
    def testTakeAlongAxisIssue1521(self):
        ...
        def f(x):
            y = x * lnp.arange(3.0).reshape((1, 3))
