...
class TrainingTest(absltest.TestCase):
    ...
    def test_train_save_restore_transformer(self):
        ...
        def _make_model_and_session():
            m = transformer.TransformerLM(
                vocab_size, d_model=4, d_ff=4, n_layers=1, n_heads=2, dropout=0.0
            )
            ts = training.Loop(
                m,
                [task],
    ...
    def test_train_dense_layer_evals(self):
        ...
        eval_task = training.EvalTask(
            _very_simple_data(), [tl.L2Loss()]  # deliberately re-using training data
        )
        training_session = training.Loop(
            model, [task], eval_tasks=[eval_task], eval_at=lambda step_n: False
        )
        self.assertEqual(0, training_session.step)
    ...
    def test_restores_memory_efficient_from_standard(self):
        ...
        loop2 = training.Loop(
            model, [task_memeff], output_dir=tmp_dir, use_memory_efficient_trainer=True
        )
        loop2.run(2)
    ...
    def test_restores_step_sharded(self):
        ...
        loop.run(4)
        loop2 = training.Loop(
            model, [task], output_dir=tmp_dir, use_memory_efficient_trainer=True
        )
        self.assertEqual(4, loop2.step)
    ...
    def test_train_one_task_eval_two_tasks(self):
        ...
        export_prefixes = [task.export_prefix for task in training_session.eval_tasks]
        self.assertCountEqual([export_prefix_1, export_prefix_2], export_prefixes)
