...
class EfficientAttentionTest(test.TestCase, parameterized.TestCase):
    ...
    def _run_forward_and_backward(self, model, inp, weights, state):
        def forward(inp, weights):
            return model.pure_fn(inp, weights, state, rng=jax.random.PRNGKey(0))
    ...
    def _test_equivalence_to_reference_code(
        ...
        for kwargs in test_kwargs:
            ...
            self.assertEqual(jax.tree_structure(ref_out), jax.tree_structure(test_out))
            self.assertEqual(
                jax.tree_structure(ref_state), jax.tree_structure(test_state)
            )
            self.assertEqual(
                jax.tree_structure(ref_inp_grad), jax.tree_structure(test_inp_grad)
            )
            self.assertEqual(
                jax.tree_structure(ref_weights_grad),
            ...
            fastmath.nested_map_multiarg(
                check_close, ref_weights_grad, test_weights_grad
            )
    ...
    def _test_fast_inference(
        ...
        ref_out, _ = ref_model.pure_fn(x, weights, state, rng=jax.random.PRNGKey(0))

        def get_slice(pytree, i):
            def get_slice_for_val(x):
                if isinstance(x, shapes.ShapeDtype):
                    return shapes.ShapeDtype(
                        shape=x.shape[:1] + (1,) + x.shape[2:], dtype=x.dtype
                    )
                else:
                    return x[:, i : i + 1]
    ...
    def test_lsh_and_pure_lsh_self_attention_equivalence(self):
        ...
        with fastmath.use_backend(fastmath.Backend.JAX):
            ...
            x = jax.random.uniform(
                jax.random.PRNGKey(0), input_shape, dtype=jnp.float32
            )
            lsh_layer_input = x
