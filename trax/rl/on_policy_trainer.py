...
class OnPolicyTrainer(base_trainer.BaseTrainer):
  ...
  def __init__(
      ...
      **kwargs):
    """Creates the OnPolicyTrainer trainer.
    ...
    """
    super(OnPolicyTrainer, self).__init__(train_env, eval_env, output_dir,
                                          **kwargs)
  ...
  def reset(self, output_dir=None):
    super(OnPolicyTrainer, self).reset(output_dir)
  ...
  def save(self):
    ...
    logging.vlog(1, 'OnPolicyTrainer epoch [% 6d]: saving model.', self.epoch)
    ppo.save_opt_state(
        self._output_dir,
