...
def permute_via_sort(val, keys, inverse_keys, axis=0):
    ...
    def permute_bwd(ik, permuted_grad):
        _, val_grad = fastmath.sort_key_val(ik, permuted_grad, dimension=axis)
...
class EfficientAttentionBase(base.Layer):
    ...
    def __init__(
        ...
        incremental=False,
        predict_mem_len=None,
        predict_drop_len=None,
        ...
        if self._incremental:
            ...
            self._predict_mem_len = predict_mem_len
            self._predict_drop_len = predict_drop_len

        if n_parallel_heads:
            if (n_parallel_heads > n_heads and n_parallel_heads % n_heads != 0) or (
                n_parallel_heads < n_heads and n_heads % n_parallel_heads != 0
            ):
                raise ValueError(
                    "n_parallel_heads must be a multiple or fraction of n_heads"
                )
            self._n_parallel_heads = n_parallel_heads
        else:
            self._n_parallel_heads = None
        self._use_python_loop = use_python_loop
        self._use_reference_code = use_reference_code

    def init_weights_and_state(self, input_signature):
        if not isinstance(input_signature, (tuple, list)):
            input_signature = (input_signature,)
        input_signature_unbatched = fastmath.nested_map(
            lambda x: type(x)(shape=x.shape[1:], dtype=x.dtype), input_signature
        )
        batch_size = int(input_signature[0].shape[0])

        weights = []
        weight_rngs = fastmath.random.split(self.rng, self._n_heads)
        for i in range(self._n_heads):
            weights.append(
                self.create_weights_unbatched(input_signature_unbatched, weight_rngs[i])
            )
        state = []
        state_rngs = fastmath.random.split(self.rng, self._n_heads * batch_size)
        for i in range(self._n_heads * batch_size):
            state.append(
                self.create_state_unbatched(input_signature_unbatched, state_rngs[i])
            )
        ...
        if self._incremental:
            mem = fastmath.nested_map(
                lambda x: np.zeros(  # pylint: disable=g-long-lambda
                    x.shape[:1] + (self._predict_mem_len,) + x.shape[2:], dtype=x.dtype
                ),
                input_signature,
            )
    ...
    def create_weights_unbatched(self, input_signature, rng):
        raise NotImplementedError("Subclasses should override create_weights_unbatched")
    ...
    def _incremental_forward_unbatched(self, *inputs, q_start, q_len, weights, state):
    ...
    def forward(self, inputs):
        ...
        if not self._use_reference_code:
        ...
        if self._incremental:
            inputs, state, q_start, new_mem, new_mem_end = self._use_predict_mem(
                inputs, state
            )
        ...
        for example_idx in range(batch_size):
            for head_idx in range(self._n_heads):
                ...
                single_state = fastmath.nested_map(
                    lambda s: s[example_idx * self._n_heads + head_idx], state
                )
                # pylint: enable=cell-var-from-loop
                if self._incremental:
                    single_out, single_new_state = self._incremental_forward_unbatched(
                        *single_inputs,
                        q_start=q_start,
                        q_len=seqlen,
    ...
    def _use_predict_mem(self, inputs, state):
        """Update input cache for fast inference."""
        mem_end, mem, state = state
        seqlen = inputs[0].shape[-2]

        if seqlen <= self._predict_drop_len and seqlen < self._predict_mem_len:
            ...
            # of tokens (self._predict_drop_tokens) will be dropped from memory if
            # needed, and then new values will be inserted into the memory.
            def roll_mem(buf):
                return np.concatenate(
                    [
                        buf[:, self._predict_drop_len :],
                        np.zeros_like(buf[:, : self._predict_drop_len]),
                    ],
                    axis=1,
                )

            do_roll_mem = mem_end + seqlen > self._predict_mem_len
            mem = fastmath.cond(
                pred=do_roll_mem,
                true_operand=mem,
            ...
            mem_end = np.where(do_roll_mem, mem_end - self._predict_drop_len, mem_end)
            ...
            assert seqlen > self._predict_drop_len or seqlen == self._predict_mem_len
                ...
                if seqlen == self._predict_mem_len:
                    ...
                    new_mem_val = np.concatenate(
                        [
                            ...
                            np.zeros(
                                inp.shape[:1]
                                + (self._predict_mem_len - inp.shape[1],)
                                + inp.shape[2:],
                                dtype=inp.dtype,
                            ),
                        ],
    ...
    def forward_and_or_backward(
        ...
        def tree_update(tree, indices, new_values):
            return fastmath.nested_map_multiarg(
                lambda x, y: fastmath.index_update(x, jax.numpy.index_exp[indices], y),
                tree,
                new_values,
            )
        ...
        if compute_grad:
            ...
            def vjp(fn, inp, *args, has_aux=False):
                ...
                return fastmath.vjp(
                    fn_closed_over_nd_inp, d_inp, *args, has_aux=has_aux
                )
        ...
        if n_parallel_heads == 1:
            ...
            def run_inner(idx, loop_val):
                ...
                example_range = example_idx_lo + np.arange(
                    n_parallel_heads // self._n_heads, dtype=np.int32
                )
                state_range = idx + np.arange(n_parallel_heads, dtype=np.int32)
                ...
                if compute_grad:
                    o_mex, backward_fn, s_mex = vjp(
                        forward_fn, i_mex, weights, has_aux=True
                    )
                ...
                if compute_output:
                    o_all = fastmath.index_add(
                        o_all, jax.numpy.index_exp[example_range], o_mex
                    )
                if update_state:
...
class SelfAttention(base.Layer):
    ...
    def __init__(
        ...
        self._incremental = mode == "predict"
        if self._incremental:
        ...
        self._n_chunks_before = n_chunks_before
        self._n_chunks_after = n_chunks_after
        self._bias = bias
        self._mode = mode
        if mode == "train":
    ...
    def _kernel_initializer(self, shape, rng):
        ...
        lim = np.sqrt(6.0 / (shape[0] + shape[1] * self._n_heads))
    ...
    def create_weights_unbatched(self, input_signature, rng):
        ...
        if self._bias:
            b_q = np.zeros(self._d_qk)
            b_v = np.zeros(self._d_v)
            if self._share_qk:
                return (w_q, w_v, w_o, b_q, b_v)
            else:
                b_k = np.zeros(self._d_qk)
    ...
    def _incremental_forward_unbatched(
        self, x, mask=None, *, q_start, q_len, weights, state, rng, update_state
    ):
        """Perform fast inference for a single batch element and head.
        ...
        if self._chunk_len is not None and q_len > self._chunk_len:
            assert q_start == 0
            assert q_len % self._chunk_len == 0
            o, _ = attend(
                q,
                k,
                v,
...
class LSHSelfAttention(base.Layer):
    ...
    def hash_vectors(self, vecs, rng, mask=None):
        ...
        # Hash vectors.
        buckets, n_buckets = hash_vecs(vecs, n_buckets_list, self._n_hashes, rng)
        ...
        offsets = np.arange(self._n_hashes, dtype=np.int32)
    ...
    def forward_unbatched(self, x, mask=None, *, weights, state, rng, update_state):
        ...
        ticker = np.arange(self._n_hashes * seqlen, dtype=np.int32)
    ...
    def _incremental_forward_unbatched(
        ...
        if q_len > 1:
            ...
            if x.shape[0] % self._chunk_len == 0:
                x_padded = x
            else:
                pad_amount = self._chunk_len - (x.shape[0] % self._chunk_len)
        ...
        def roll_buckets(buckets):
            ...
            new_buckets = np.concatenate(
                [
                    buckets,
                    np.zeros(
                        (self._n_hashes, self._predict_drop_len), dtype=buckets.dtype
                    ),
                ],
                axis=1,
            )
        ...
        q_buckets = np.reshape(q_buckets, (self._n_hashes, 2))[:, :q_len]

        unflattened_buckets = fastmath.dynamic_update_slice_in_dim(
            np.reshape(buckets, (self._n_hashes, -1)), q_buckets, q_start, axis=1
        )
        ...
        kv_priorities = np.where(
            arange_seqlen > (q_start + q_len), -(seqlen + arange_seqlen), arange_seqlen
        )
        ...
        kv_indices = kv_indices[
            -self._n_hashes * self._chunk_len * (1 + self._n_chunks_before) :
        ]
        assert self._n_chunks_after == 0
...
class PureLSHSelfAttention(base.Layer):
    ...
    def init_weights_and_state(self, input_signature):
        ...
        expected_inputs = 3 if self._masked else 2
        if not (
            isinstance(input_signature, (tuple, list))
            and len(input_signature) == expected_inputs
        ):
            raise ValueError(
        ...
        assert batch_x_heads % self._n_heads == 0
        batch = batch_x_heads // self._n_heads

        query_signature_unbatched = fastmath.nested_map(
            lambda x: type(x)(shape=x.shape[1:], dtype=x.dtype), qk_signature
        )

        state_rngs = fastmath.random.split(self.rng, batch_x_heads)
        state = [
            self.create_state_unbatched(query_signature_unbatched, rng)
    ...
    def forward_unbatched(self, qk, v, mask=None, *, state, rng, update_state):
        ...
        if update_state:
            ...
            buckets, _ = state
            if self._max_length_for_buckets:
                buckets = buckets[: self._n_hashes * seqlen]
    ...
    def _incremental_forward_unbatched(
        self, qk, v, mask=None, *, q_start, q_len, state, rng, update_state
    ):
        x = (qk, v)
        length = x[0].shape[0]
        assert (
        ...
        if q_len > 1:
            if isinstance(q_start, int):
                assert q_start == 0, "Chunks larger than 1 only work at start for now."
            if length % self._chunk_len == 0:
                x_padded = x
            else:
                pad_amount = self._chunk_len - (length % self._chunk_len)
        ...
        kv_priorities = np.where(
            arange_seqlen > (q_start + q_len), -(length + arange_seqlen), arange_seqlen
        )
    ...
    def forward_and_or_backward(
        ...
        if self._masked:
            ...
            qk, v = inputs
            mask = None
            batch_size = qk.shape[0] // self._n_heads
        ...
        def run_inner(idx, loop_val):
            ...
            example_idx = idx // self._n_heads
            unused_head_idx = idx % self._n_heads
            ...
            def forward_fn(i_h):
                return forward_unbatched(*i_h, state=fastmath.stop_gradient(s_h))
...
def _ProjectAndSplitHeads(  # pylint: disable=invalid-name
    ...
    if weights_format == "model" and num_weights == 3:
        return cb.Serial(
            ...
            cb.Parallel(
                attention.SplitIntoHeads(n_heads), attention.SplitIntoHeads(n_heads)
            ),  # qk,   v
        )
    ...
    raise NotImplementedError(
        "TODO(afrozm): Implement this when we want to use "
...
class MixedLSHSelfAttention(base.Layer):
    ...
    def __init__(
        ...
        **pure_lsh_implementation_kwargs,
        ...
        self._lsha = PureLSHSelfAttention(
            n_heads=n_heads,
            d_qk=d_qk,
            d_v=d_v,
            causal=causal,
        ...
        if causal:
            ...
            def _add_heads_to_mask(m):
                ...
                m_with_heads = np.broadcast_to(
                    m_with_heads, (m.shape[0], n_heads, m.shape[1])
                )
                return np.reshape(m_with_heads, (-1, 1, m.shape[1]))

            preprocess = core.Fn(
                "dup_shared_qk_and_make_mask",
                lambda q, v, m: (q, q, v, _add_heads_to_mask(m)),
                n_out=4,
            )
        self._stda = cb.Serial(
            preprocess, pure_attn(dropout=attention_dropout, mode=mode)
        )
...
class PureLSHSelfAttentionWrapper(cb.Serial):
    ...
    def __init__(
        ...
        **pure_lsh_implementation_kwargs,
        ...
        self._attn = pure_lsh_implementation(
            n_heads=n_heads,
            d_qk=d_qk,
            d_v=d_v,
            causal=causal,
    ...
    def forward_and_or_backward(
        ...
        qkv_output, qkv_vjp_fn, unused_qkv_new_state = fastmath.vjp(
            self._qkv.pure_fn, inputs, weights[0], state[0], rngs[0], has_aux=True
        )
        ...
        # Now backward.
        if self._dense is not None:
            dense_grads_inputs, dense_grads_weights, _, _ = dense_vjp_fn(output_grad)
        else:
            dense_grads_inputs = output_grad
        merge_grads_inputs, merge_grads_weights, _, _ = merge_vjp_fn(dense_grads_inputs)
        ...
        (attn_output, _, attn_grads_inputs) = self._attn.forward_and_or_backward(
            ...
            state[1],
            rngs[1],
            output_grad=merge_grads_inputs,
            compute_output=True,
            update_state=False,
        )
        ...
        if self._dense is None:
            grads_weights = (qkv_grads_weights, (), merge_grads_weights)
        else:
            grads_weights = (
                qkv_grads_weights,
                (),
...
class EncDecAttention(EfficientAttentionBase):
    ...
    def create_weights_unbatched(self, input_signature, rng):
        ...
        rng_q, rng_k, rng_v, rng_o = fastmath.random.split(rng, 4)
        w_q = self._kernel_initializer((d_model, self._d_qk), rng_q)
        w_k = self._kernel_initializer((d_kv_antecedent, self._d_qk), rng_k)
        w_v = self._kernel_initializer((d_kv_antecedent, self._d_v), rng_v)
        w_o = np.transpose(self._kernel_initializer((d_model, self._d_v), rng_o))
        return (w_q, w_k, w_v, w_o)
