...
def MergeLastTwoAxes():
    return tl.Fn(
        "MergeLastTwoAxes", lambda x: jnp.reshape(x, tuple(x.shape)[:-2] + (-1,))
    )
...
def ModularCausalAttention(
    ...
    def ProcessingLayer():
        ...
        return LocallyConnectedDense(
            n_modules, d_feature // n_modules, kernel_size=kernel_size
        )
    ...
    return tl.ConfigurableAttention(
        ...
        qkv_attention_layer=tl.DotProductCausalAttention(
            dropout=dropout, max_inference_length=max_inference_length, mode=mode
        ),
    )
    ...
    def forward(self, x):
        ...
        if self._mode == "predict":
            ...
            if self._portal_mask is not None and "init" in self.state[1]:
                ...
                self.state = (
                    fastmath.dynamic_slice_in_dim(
                        x,
                        x.shape[1] - (self._n_items_to_remember + count_padding),
                        self._n_items_to_remember,
                        axis=1,
                    ),
                    {"forward": ()},
                )
...
@assert_shape("...a->...b")
def LocallyConvDense(n_modules, n_units, mode, kernel_size=1, length_kernel_size=1):
...
def ConvCausalAttention(
    ...
    def ProcessingLayer():
        assert d_feature % n_modules == 0
        return LocallyConvDense(
            n_modules, d_feature // n_modules, mode=mode, kernel_size=kernel_size
        )
...
def LowRankDense(n_units, d_lowrank):
    return tl.Serial(tl.Dense(d_lowrank), tl.Dense(n_units))
...
def EinsumDense(d_input, d_output, use_bias):
    ...
    layers = [
        ...
        tl.Fn(
            "EinsumDense",
            (
                lambda kernel, embeds: jnp.einsum(  # pylint: disable=g-long-lambda
                    "xd,...d->...x", kernel, embeds
                )
            ),
        ),
    ]
    if use_bias:
        layers.extend(
            [tl.Weights(init.RandomNormalInitializer(1e-6), [d_output]), tl.Add()]
        )
...
def RandomLayer(layer_a, layer_b, prob_a):
    ...
    condition = tl.Serial(
        tl.RandomUniform(), tl.Fn("SmallerThan", lambda x: x < prob_a)
    )
    return tl.Cond(condition, layer_a, layer_b)


@assert_shape("...a->...b")
def SparseDenseWithOptions(
    n_units,
    d_input=None,
    sparsity_type=None,
    sparsity=0,
    ...
    if sparsity_type == "mult":
        return FactoredDense(
            sparsity, d_input, n_units, use_bias=use_bias, use_bfloat16=use_bfloat16
        )
...
def FactoredDense(n_modules, d_in, d_out, use_bias=True, use_bfloat16=False):
    ...
    if d_out % n_modules != 0:
        raise ValueError(
            f"Value d_out ({d_out}) must be a multiple of arg "
    ...
    def GatingVectors():
        return tl.Weights(
            init.RandomNormalInitializer(stddev=0.5),
    ...
    def Bias():
        return (
            tl.Weights(
                init.RandomNormalInitializer(1e-6),
                shape=[d_out],
                use_bfloat16=use_bfloat16,
            ),
        )
...
def MultiplicativeModularSparseDense(sparsity, d_feature):
    ...
    return tl.Serial(
        ...
        tl.Weights(
            init.RandomNormalInitializer(stddev=0.5), shape=[sparsity, d_feature]
        ),
        ...
        tl.Weights(
            functools.partial(init.GlorotUniformInitializer(), nonreceptive_dims=[0]),
        ...
        tl.Fn(
            "SparseDenseEinsum",
            (
                lambda kmod, kmult, multiplier, embeds: jnp.einsum(  # pylint: disable=g-long-lambda
                    "hxo,dx,hd,...d->...ho", kmod, kmult, multiplier, embeds
                )
            ),
        ),
        MergeLastTwoAxes(),
...
@assert_shape("bld->bld")
def MultiplicativeCausalAttention(
    d_feature,
    n_heads=1,
    sparsity=None,
    dropout=0.0,
...
def MultiplicativeConvCausalAttention(
    ...
    if "mult" in output_layer_type:
        output_layers.append(FactoredDense(sparsity, d_feature, d_feature))
...
class FavorAttention(base.Layer):
    ...
    def __init__(
        ...
        if self._use_approximate_softmax:
            ...
            self._projection_matrix = self.get_2d_array(
                rng=rng,
                n_rows=self._n_random_features,
    ...
    def nonnegative_softmax_kernel_feature_creator(self, x, is_query):
        ...
        data_thick_random_matrix = jnp.zeros(data_mod_shape) + self._projection_matrix

        data_dash = jnp.einsum(
            "Bij, Bkj -> Bik", data_normalizer * x, data_thick_random_matrix
        )
...
class CausalFavorAttention(base.Layer):
    ...
    def forward(self, inputs):
        def favor_numerator_fwd(init_prefix_sum_value, query_prime, key_prime, value):
            def body(p, qkv):
                (q, k, v) = qkv
            ...
            p, w = fastmath.scan(
                body, init_prefix_sum_value, (query_prime, key_prime, value)
            )
        ...
        def favor_numerator(init_prefix_sum_value, query_prime, key_prime, value):
            w, _ = favor_numerator_fwd(
                init_prefix_sum_value, query_prime, key_prime, value
            )
            return w
        ...
        def favor_denominator_fwd(init_prefix_sum_value, query_prime, key_prime):
            def body(p, qk):
                q, k = qk
                p += k
        ...
        def favor_denominator(init_prefix_sum_value, query_prime, key_prime):
            r, _ = favor_denominator_fwd(init_prefix_sum_value, query_prime, key_prime)
            return r
        ...
        w = favor_numerator(
            init_prefix_sum_value_numerator,
            jnp.moveaxis(query_prime, 1, 0),
            jnp.moveaxis(key_prime, 1, 0),
            jnp.moveaxis(value, 1, 0),
        )
        r = favor_denominator(
            init_prefix_sum_value_denominator,
            jnp.moveaxis(query_prime, 1, 0),
            jnp.moveaxis(key_prime, 1, 0),
        )
        w = jnp.moveaxis(w, 0, 1)
...
def CausalFavor(
    d_feature, n_heads=1, dropout=0.0, numerical_stabilizer=0.001, mode="train"
):
    """Returns a layer that maps activations to activations, with causal masking.
    ...
    return tl.ConfigurableAttention(
        ...
        core.Dense(d_feature),
        n_heads=n_heads,
        qkv_attention_layer=tl.CausalFavorAttention(numerical_stabilizer, mode),
    )
...
class _RememberInReverse(base.Layer):
    ...
    def init_weights_and_state(self, input_signature):
        ...
        self.weights = ()
        self.state = (
            jnp.zeros(input_signature.shape, dtype=jnp.int32),
            {"running_second_time": ()},
        )
...
class _RecallQuantMaskInReverse(base.Layer):
    ...
    def forward(self, x):
        if (
            self._remember_layer.state
            and "running_second_time_yes" in self._remember_layer.state[1]
        ):
    ...
    def forward(self, x):
        ...
        if self._also_return_nondiscrete_output:
            ...
            if self._temperature == 0.0:
                ...
                u = fastmath.random.uniform(
                    self.rng, mask.shape, jnp.float32, 1e-6, 1.0 - 1e-6
                )
                g = -jnp.log(-jnp.log(u))
        ...
        if self._mode == "train":
            ...
            quant_mask = jnp.where(
                recalled_quant_mask == -1, quant_mask, recalled_quant_mask
            )
    ...
    def forward(self, x):
        ...
        if self._mode == "train":
            ...
            if self._multiply_by_controller_output:
                ...
                mask_mult = jnp.where(
                    select < self._quant_prob, mask, jnp.ones_like(mask)
                )
...
def SparseFF(
    ...
    main = [
        _SparseFFMain(
            ...
            use_bfloat16=use_bfloat16,
            big_weights_in_bfloat16=big_weights_in_bfloat16,
            mode=mode,
            kernel_initializer=kernel_initializer,
            bias_initializer=bias_initializer,
...
class SwitchSparseFF(base.Layer):
    ...
    def forward(self, x):
        ...
        if self._mode == "predict" and batch_size == 1:
            ...
            selected_mask_flat = mask_flat[np.arange(selected_flat.size), selected_flat]
            # This implementation mimicks inference for batch_size 1.
            start_idx = selected_experts[0] * self._n_elements_in_block
            # w1 is [d_model, d_ff], w is [d_model, n_elements_in_block]
            w = fastmath.dynamic_slice(
