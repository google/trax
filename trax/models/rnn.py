...
def RNNLM(
    ...
    if n_layers != 2:  # TODO(jonni): Remove n_layers arg, if it can't vary?
        raise ValueError(
            f"Number of layers must be set to 2; instead got" f" {n_layers}."
        )

    def MultiRNNCell():
        """Multi-layer RNN cell."""
        return tl.Serial(
            tl.Parallel(tl.Select([0]), tl.Split(n_items=n_layers)),
            tl.SerialWithSideOutputs(
                [rnn_cell(n_units=d_model) for _ in range(n_layers)]
            ),
            tl.Parallel(tl.Select([0]), tl.Concatenate(n_items=n_layers)),
        )
    ...
    return tl.Serial(
        ...
        tl.Branch(tl.Select([0]), zero_state),
