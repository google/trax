...
@assert_shape("...s->...sv")
def EveryOtherLayerDropTransformerLM(
    vocab_size,
    d_model=512,
    d_ff=2048,
    n_layers=6,
    ...
    def ConditionedBlock(current_layer_num):
        return tl.Serial(
            ...
            tl.Cond(
                ...
                LargerThan(skip_fraction if skip_mode_fun(current_layer_num) else 0.0),
                # then: run block
                tl.Serial(
                    transformer._DecoderBlock(  # pylint: disable=g-complex-comprehension,protected-access
                        d_model, d_ff, n_heads, dropout, [], mode, ff_activation
    ...
    return tl.Serial(
        ...
        embedder,
        # stack: embedding
        tl.RandomUniform(0.0, 1.0, sync=True),
        # stack: n_layers_to_keep, embedding
        tl.Swap(),
        # stack: embedding, n_layers_to_keep
        [ConditionedBlock(i) for i in range(n_layers)],
...
def LayerDropTransformerLM(
    ...
    if eval_skip_fraction == "every_other":
        ...
        eval_skip_fraction = [
            (1.0 if i % int(1.0 / skip_fraction[i]) == 0 else 0.0)
            if skip_fraction[i] != 0
            else 0.0
    ...
    if len(eval_skip_fraction) != n_layers:
        raise ValueError(
            "n_layers ({}) must be equal to len(eval_skip_fraction) ({})".format(
                n_layers, len(eval_skip_fraction)
            )
        )
    ...
    def ConditionedBlock(current_layer_num):
        return tl.Serial(
            # stack: embedding
            tl.RandomUniform(0.0, 1, sync=True),
            # stack: random_uniform, embedding
            tl.Cond(
                # if random_uniform > skip_fraction
                LargerThan(
                    skip_fraction[current_layer_num]
                    if mode == "train"
