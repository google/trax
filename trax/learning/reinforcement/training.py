...
class Agent:
    ...
    def save_to_file(
        self, file_name="reinforcement.pkl", task_file_name="trajectories.pkl"
    ):
        ...
        for eval_t in self._eval_temperatures:
            ...
            ] = self._avg_returns_temperatures[eval_t]
    ...
    def policy(self, trajectory, temperature=1.0):
        """Policy function that allows to play using this trainers.

        Args:
          trajectory: an instance of trax.reinforcement.task.Trajectory
    ...
    def run(self, n_epochs=1, n_epochs_is_total_epochs=False):
        ...
        with self._open_summary_writer() as sw:
            ...
            for _ in range(n_epochs_to_run):
                ...
                if sw is not None:
                    sw.scalar(
                        "timing/collect", time.time() - cur_time, step=self._epoch
                    )
                    sw.scalar("reinforcement/avg_return", avg_return, step=self._epoch)
                    if self._n_eval_episodes > 0:
                        for steps in self._eval_steps:
                            for eval_t in self._eval_temperatures:
                                sw.scalar(
                                    "reinforcement/avg_return_temperature%.2f_steps%d"
                                    % (eval_t, steps),
                                    self._avg_returns_temperatures[eval_t][steps][-1],
                                    step=self._epoch,
                                )
                    sw.scalar(
                        "reinforcement/n_interactions",
                        self.task.n_interactions(),
                        step=self._epoch,
                    )
                    sw.scalar(
                        "reinforcement/n_interactions_per_second",
                    ...
                    sw.scalar(
                        "reinforcement/n_trajectories",
                        self.task.n_trajectories(),
                        step=self._epoch,
                    )
                    sw.flush()
...
class PolicyAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Configures the policy trainers.
        ...
        Args:
          ...
          policy_evals_per_epoch: number of policy trainers evaluations per RL epoch
              - only affects metric reporting.
          policy_eval_steps: number of policy trainers steps per evaluation - only
        ...
        """
        super().__init__(
            task, n_eval_episodes=n_eval_episodes, output_dir=output_dir, **kwargs
        )
...
def remaining_evals(cur_step, epoch, train_steps_per_epoch, evals_per_epoch):
    """Helper function to calculate remaining evaluations for a trainers.

    Args:
      cur_step: current step of the supervised trainers
      epoch: current epoch of the RL trainers
      train_steps_per_epoch: supervised trainers steps per RL epoch
      evals_per_epoch: supervised trainers evals per RL epoch
    ...
    if done_steps_this_epoch < 0:
        raise ValueError(
            "Current step (%d) < previously done steps (%d)." % (cur_step, prev_steps)
        )
    train_steps_per_eval = train_steps_per_epoch // evals_per_epoch
    if done_steps_this_epoch % train_steps_per_eval != 0:
        raise ValueError(
            "Done steps (%d) must divide train steps per eval (%d)."
...
class LoopPolicyAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
          task: Instance of trax.reinforcement.task.RLTask.
...
@gin.configurable
def sharpened_network_policy(temperature, temperature_multiplier=1.0, **kwargs):
    ...
    Returns:
      ...
      training.
    """
    return network_policy(temperature=(temperature_multiplier * temperature), **kwargs)
...
def network_policy(
    ...
    Args:
      ...
      policy_distribution: an instance of trax.reinforcement.distributions.Distribution
      loop: trax.supervised.training.Loop used to train the policy network
      trajectory_np: an instance of trax.reinforcement.task.TimeStepBatch
    ...
    assert pred.shape == (
        1,
        trajectory_np.observation.shape[0],
        policy_distribution.n_inputs,
    )
    # Pick element 0 from the batch (the only one), last (current) timestep.
    pred = pred[0, -1, :]
...
class ValueAgent(Agent):
    ...
    def __init__(
        ...
        exploration_rate=functools.partial(
            ...
            constant=1.0,  # pylint: disable=redefined-outer-name
            decay_factor=0.99,
            steps_per_decay=1,
            minimum=0.1,
        ),
        ...
        **kwargs,
        ...
        Args:
          ...
          value_evals_per_epoch: number of policy trainers evaluations per RL epoch
              - only affects metric reporting.
          value_eval_steps: number of policy trainers steps per evaluation - only
              affects metric reporting.
...
class DQN(ValueAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DQN, self).__init__(
            task=task, max_slice_length=max_slice_length, **kwargs
        )
    ...
    def value_batches_stream(self):
        ...
        for np_trajectory in self._task.trajectory_batch_stream(
            ...
            n_step_returns = values_max[:, : -self._margin] + self._advantage_estimator(
    ...
    def policy(self, trajectory, temperature=1):
        ...
        if (
            np.random.random_sample() < self._exploration_rate(self._epoch)
            and temperature == 1
        ):
