...
class TerraformerTest(parameterized.TestCase):
    ...
    def test_terraformer_one_step(self):
        ...
        lsh_self_attention = functools.partial(
            self._lsh_self_attention_fn(), chunk_len=chunk_len, n_buckets=n_buckets
        )
        ...
        model = terraformer.ConfigurableTerraformer(
            ...
            encoder_decoder_attention_type=[timebin_self_attention, lsh_self_attention],
        ...
        def mock_training_step(x, weights, state, rng):
            ...
            gradients, (new_state, logits) = fastmath.grad(
                compute_mock_loss, has_aux=True
            )(weights)
