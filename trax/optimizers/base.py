...
class Optimizer:
    ...
    def __init__(self, learning_rate=0.01, clip_grad_norm=None, **init_opt_params):
    ...
    def tree_update(
        self, step, grad_tree, weight_tree, slots, opt_params, store_slots=True
    ):
    ...
    def _update_and_check(self, step, grads, weights, slots, opt_params):
        ...
        new_weights, new_slots = self.update(step, grads, weights, slots, opt_params)
...
def l2_norm(tree):
    ...
    if fastmath.is_backend(fastmath.Backend.JAX):
        ...
        norm = jnp.sqrt(sum(jnp.tensordot(x, x)) for x in leaves)
