...
flags.DEFINE_string("checkpoint_dir", "", "Path to model checkpoint.")
flags.DEFINE_integer("max_answer_len", 1024, "Maximum length of answers to produce.")
...
flags.DEFINE_float(
    "autoregressive_sample_temp", 1, "The temperature for autoregressive sampling."
)
flags.DEFINE_integer("n_beams", 4, "How many beams to use in beam search.")
flags.DEFINE_string(
    "output_dir",
    "",
    "Path to the output directory where articles, abstracts, "
    "and predictions would be stored.",
)
flags.DEFINE_integer("starting_example", 0, "Example index for starting decoding.")
flags.DEFINE_integer(
    "reload_after", 1000, "Reload checkpoint after reload_after examples."
)
...
def prepare_model(model_file, batch_size=1):
    ...
    model_reference = gin.query_parameter("trax.supervised.trainer_lib.train.model")
...
def main(argv):
    ...
    with tf.io.gfile.GFile(json_to_write + "_" + str(FLAGS.starting_example), "w") as w:
