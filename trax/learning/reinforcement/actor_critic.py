...
class ActorCriticAgent(rl_training.PolicyAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
          ...
          value_evals_per_epoch: Number of value trainers evaluations per RL epoch.
              Every evaluation, we also synchronize the weights of the target
              network.
          value_eval_steps: Number of value trainers steps per evaluation; only
    ...
    def _aggregate_values(self, values, aggregate, act_log_probs):
        ...
        if self._q_value:
            assert values.shape[:2] == (self._value_batch_size, self._q_value_n_samples)
            if aggregate == "max":
                # max_a Q(s, a)
...
class AdvantageBasedActorCriticAgent(ActorCriticAgent):
    ...
    def policy_inputs(self, trajectory, values):
        ...
        if len(advantages.shape) != 2:
            raise ValueError(
                "Advantages are expected to have shape "
        ...
        if obs.shape[0:2] != advantages.shape:
            raise ValueError(
                ...
                "as in advantages, %s != %s" % (obs.shape[0:2], advantages.shape)
            )
        if dist_inputs.shape[:2] != advantages.shape:
            raise ValueError(
                "First 2 dimensions of dist_inputs should be the same "
                "as in advantages, %s != %s" % (dist_inputs.shape[:2], advantages.shape)
            )
        if mask.shape != advantages.shape:
            raise ValueError(
                "Mask and advantages shapes should be the same"
...
class LoopActorCriticAgent(rl_training.Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._policy_dist = distributions.create_distribution(self.task.action_space)
    ...
    def policy_loss_given_log_probs(self):
        ...
        def f(log_probs, advantages, old_log_probs, mask):
            ...
            if log_probs.shape != advantages.shape:
                raise ValueError(
                    "New log-probs and advantages shapes "
                    "should be the same, %s != %s" % (log_probs.shape, advantages.shape)
                )
            if log_probs.shape != mask.shape:
                raise ValueError(
                    "New log-probs and mask shapes should be the same"
        ...
        def f(new_log_probs, advantages, old_log_probs, mask):
            ...
            if new_log_probs.shape != advantages.shape:
                raise ValueError(
                    ...
                    % (new_log_probs.shape, advantages.shape)
                )
            if new_log_probs.shape != old_log_probs.shape:
                raise ValueError(
                    "New log-probs and old log-probs shapes "
                    "should be the same, %s != %s"
            ...
            if advantages.shape != probs_ratio.shape:
                raise ValueError(
                    "New log-probs and old log probs shapes "
                    "should be the same, %s != %s"
                    % (advantages.shape, probs_ratio.shape)
                )
            unclipped_objective = probs_ratio * advantages
            clipped_objective = (
                jnp.clip(probs_ratio, 1 - self._epsilon, 1 + self._epsilon) * advantages
            )

            if unclipped_objective.shape != probs_ratio.shape:
                raise ValueError(
                    "unclipped_objective and clipped_objective shapes "
                    "should be the same, %s != %s"
                    % (unclipped_objective.shape, clipped_objective.shape)
                )
            ...
            if ppo_objective.shape != mask.shape:
                raise ValueError(
                    "ppo_objective and mask shapes "
                    "should be the same, %s != %s" % (ppo_objective.shape, mask.shape)
                )

            ppo_loss = -jnp.sum(ppo_objective * mask) / jnp.sum(mask)
            entropy_vec = self._policy_dist.entropy(new_log_probs) * self._entropy_coeff
            entropy_loss = jnp.mean(entropy_vec)
...
class LoopAWR(LoopActorCriticAgent):
    ...
    def __init__(self, task, model_fn, beta=1.0, w_max=20, **kwargs):
        ...
        super().__init__(task, model_fn, policy_weight_fn=policy_weight_fn, **kwargs)


def SamplingAWRLoss(
    beta,
    w_max,
    thresholds,  # pylint: disable=invalid-name
    reweight=False,
    sampled_all_discrete=False,
):
    """Definition of the Advantage Weighted Regression (AWR) loss."""
...
class SamplingAWR(AdvantageBasedActorCriticAgent):
    ...
    def __init__(
        self, task, beta=1.0, w_max=20.0, thresholds=None, reweight=False, **kwargs
    ):
        """Configures the AWR Trainer."""
        self._beta = beta
        self._w_max = w_max
