...
_tf_nn_APIs = {
    1: [tf.nn.conv1d, tf.nn.conv1d_transpose],
...
def most_precise_int_dtype(x):
    ...
    if i in (0, len(_int_dtype_lower_bounds)):
        raise ValueError(f"Integer {x} is out of bounds")
...
def grad(f, has_aux=False):
    ...
    def check_loss_shape(np_loss):
        ...
        # TensorFlow 1.x has
        # TensorFlow 2.x does not contain such method as is_compatible_with.
        # We can change it to compare the shape with ()
        if not np_loss.shape == ():
            raise ValueError(
...
def logsumexp(x, axis=None, keepdims=None):
    ...
    return tf_np.asarray(
        tf.math.reduce_logsumexp(input_tensor=x, axis=axis, keepdims=keepdims)
    )
...
def scan(f, init, xs, length=None, reverse=False):
    ...
    def body(i, safe_carry, ys_ta):
        ...
        ys_ta = tf.nest.map_structure(
            lambda y_ta, y: (y_ta.write(i_, y) if y is not None else y_ta), ys_ta, ys
        )
        i = i + 1
    ...
    xs_spec = tf.nest.map_structure(
        lambda t: tf.TensorSpec(t.shape[1:], t.dtype) if t is not None else None, xs
    )
    ...
    ys_ta = tf.nest.map_structure(
        lambda y: tf.TensorArray(
            y.dtype if y is not None else tf.float32,  # pylint: disable=g-long-lambda
            size=length,
            dynamic_size=False,
        ),
        ys_spec,
    ...
    _, safe_carry, ys_ta = tf.while_loop(
        lambda i, *_: i < length, body, (0, safe_init, ys_ta), maximum_iterations=length
    )
    carry = from_safe(safe_carry)
...
def _get_dynamic_indices(operand, start_indices, slice_sizes):
    ...
    tf.debugging.Assert(
        tf.reduce_all(slice_sizes <= operand_shape), [slice_sizes, operand_shape]
    )
    if rank == 0:
        return None
...
def _key2seed(a):
    ...
    def int64_to_int32s(a):
        ...
        snd = tf.cast(tf.bitwise.right_shift(a, tf.constant(32, tf.uint64)), tf.uint32)
        a = [fst, snd]
...
def _seed2key(a):
    ...
    def int32s_to_int64(a):
        ...
        a = tf.bitwise.bitwise_or(
            ...
            tf.bitwise.left_shift(tf.cast(a[1], tf.uint64), tf.constant(32, tf.uint64)),
        )
...
def psum(tensors, axis_name=None):
    return tf.nest.map_structure(functools.partial(_psum, axis_name=axis_name), tensors)
...
def pmap(f, axis_name=None, devices=None):
    ...
    def wrapper(*args):
        ...
        for i in range(len(flattened_results[0])):
            ...
            for j, device in enumerate(devices):
                assert isinstance(
                    flattened_results[j][i], tf.Tensor
