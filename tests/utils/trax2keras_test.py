...
from trax.models import mlp, transformer
from trax.utils import trax2keras
from trax.utils.trax2keras import read_values, to_arrays, to_tensors
...
def dummy_inputs(rng, input_sig):
    def f(sig):
        ...
        if onp.issubdtype(sig.dtype, onp.integer):
            ...
            minval = 0
            maxval = 1
        return rng.uniform(shape=shape, dtype=sig.dtype, minval=minval, maxval=maxval)
...
class Trax2KerasTest(tf.test.TestCase, parameterized.TestCase):
    @parameterized.named_parameters(
        [
            {
                "testcase_name": "_%s_%s_%s_%s_%s_%s"
    ...
    def testTrain(
        ...
        with trax.fastmath.use_backend("tensorflow-numpy"):
            make_trax_layer, input_shapes_no_batch, dtype, allow_none_batch = _LAYERS[
                layer_id
            ]
