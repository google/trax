...
class DecodingTimingTest(test.TestCase):
    def _terraformer_decoding_time(self, settings):
        ...
        def _causal_attention_fn():
            ...
            return functools.partial(
                attn_layer, max_inference_length=2 * max_len, **attn_kwargs
            )
        ...
        if min(elapsed_times[2:]) * 2 < max(elapsed_times[2:]):
            print(
                ...
                "or model size should be increased.".format(settings, elapsed_times)
            )
    ...
    def test_autoregressive_sample_terraformer_timing(self):
        ...
        for override_settings in sweep_settings[template_to_use]:
            ...
            size, elapsed_times, peak_memory = self._terraformer_decoding_time(settings)
            ...
            message = (
                ...
                "\n\n".format(
                    ...
                    peak_memory / 1024**3,
                    after_memory / 1024**3,
                    model_memory_gigabytes,
                    elapsed_times,
                    encoding_time,
                    decoding_time_10,
                    decoding_time_diff_percent,
                )
            )
            print(message)
