...
class Serial(base.Layer):
    ...
    def __init__(self, *sublayers, name=None, sublayers_to_print=None):
        super().__init__(name=name, sublayers_to_print=sublayers_to_print)
    ...
    def init_weights_and_state(self, input_signature):
        ...
        for sublayer in self.sublayers:
            ...
            weights_or_cache_marker, state_or_cache_marker = sublayer.init(
    ...
    def _validate_forward_inputs(self, xs):
        if not isinstance(xs, (tuple, list)) and self._n_in != 1:
            raise TypeError(
                f"Serial.forward input must be a tuple or list; "
...
class Parallel(base.Layer):
    ...
    def init_weights_and_state(self, input_signature):
        ...
        inits = [
            layer.init(signature, use_cache=True)
...
class Scan(base.Layer):
    ...
    def forward(self, inputs):
        ...
        ys, (carry, new_state, _) = _scan(
            scannable_fn, xs, init, axis=self._axis, remat=self._remat
        )
    ...
    def init_weights_and_state(self, input_signature):
        ...
        if n_carry == 0:
            if isinstance(input_signature, (list, tuple)):
                layer_sig = [
                    ShapeDtype(_shape_without_axis(x, self._axis), x.dtype)
                    for x in input_signature
                ]
                layer_sig = tuple(layer_sig)
            else:
                layer_sig = ShapeDtype(
                    _shape_without_axis(input_signature, self._axis),
            ...
            xs_slices = [
                ShapeDtype(_shape_without_axis(x, self._axis), x.dtype) for x in xs
            ]
...
class Cond(base.Layer):
    ...
    def __init__(self, cond, true, false=None, name=None):
        ...
        if true.n_in != false.n_in:
            raise ValueError(
                "true.n_in and false.n_in must be equal: true:{}->{} ; false:{}->{}".format(
                    true.n_in, true.n_out, false.n_in, false.n_out
                )
            )
        if true.n_out != false.n_out:
            raise ValueError(
                "true.n_out and false.n_out must be equal: true:{}->{} ; false:{}->{}".format(
                    true.n_in, true.n_out, false.n_in, false.n_out
                )
            )
    ...
    def init_weights_and_state(self, input_signature):
        ...
        weights_or_cache_marker, state_or_cache_marker = self._cond.init(
            inputs, use_cache=True
        )
        weights.append(weights_or_cache_marker)
        states.append(state_or_cache_marker)
    ...
    def forward(self, xs):
        ...
        if len(weights) != 3:
            raise ValueError(
                "number of weights ({}) not equal to 3".format(len(weights))
            )
        if len(layers_state) != 3:
            raise ValueError(
                "length of state ({}) not equal to 3".format(len(layers_state))
            )
    ...
    def reshape_to_chunks(x):
        ...
        if chunk_batch % size != 0:
            if pass_unchunkable:
                ...
                raise ValueError(
                    f"Chunk size {size} must divide batch " f"size {chunk_batch}"
                )
...
def Branch(*layers, name="Branch"):
    ...
    return Serial(
        Select(_deep_flatten(indices)),
        parallel_layer,
        name=name,
        sublayers_to_print=layers,
    )
...
def SerialWithSideOutputs(layers, n_side_outputs=1):
    ...
    for layer, n_side_output in zip(layers, n_side_outputs):
        ...
        move_back_indices += [
            i + layer.n_out for i in range(cur_stack_size - layer.n_out)
        ]
        # Finally the indices we move.
        move_back_indices += [
            i + layer.n_out - n_side_output for i in range(n_side_output)
        ]
        # Swap them on stack.
...
def Add():
    """Adds two tensors."""
    return Fn("Add", lambda x0, x1: jnp.add(x0, x1))


def SubtractTop():
    """Subtracts the first tensor from the second."""
    return Fn("SubtractTop", lambda x0, x1: jnp.subtract(x1, x0))


def Multiply():
    """Multiplies two tensors."""
    return Fn("Multiply", lambda x0, x1: jnp.multiply(x0, x1))
...
class BatchLeadingAxes(base.Layer):
    ...
    def __init__(self, layer, n_last_axes_to_keep=1):
        if layer.n_out != 1:
            raise ValueError(
                "BatchLeadingAxes currently only works for layers with "
    ...
    def init_weights_and_state(self, input_signature):
        ...
        for sub_input_signature in input_signature:
            ...
            batched_signature.append(
                ShapeDtype(batched_shape, sub_input_signature.dtype)
            )
        if self._n_in == 1:
...
def _ensure_flat(layers):
    ...
    for obj in layers:
        if not isinstance(obj, base.Layer):
            raise ValueError(
                f"Found non-layer object ({obj}) type ({type(obj)}) ({type(obj)}) in layers: {layers}"
            )
    return layers
