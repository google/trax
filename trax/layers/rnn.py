...
def MakeZeroState(depth_multiplier=1):
    ...
    def f(x):  # pylint: disable=invalid-name
        if len(x.shape) != 3:
            raise ValueError(
                f"Layer input should be a rank 3 tensor representing"
                f" (batch_size, sequence_length, feature_depth); "
                f"instead got shape {x.shape}."
            )
        return jnp.zeros(
            (x.shape[0], depth_multiplier * x.shape[-1]), dtype=jnp.float32
        )
...
def ScanSRUCell(mode, monkey_patched_mask=None):
    ...
    return cb.Serial(
        ...
        cb.Scan(
            base.Fn("MaskedInnerSRUCell", masked_inner_sru_cell, n_out=2),
            axis=1,
            mode=mode,
        ),
    )
...
def _AddSigmoidBias(sigmoid_bias):
    return base.Fn("AddSigmoidBias({sigmoid_bias})", lambda x: x + sigmoid_bias)
