...
suppress_deprecated_indexing_warnings = partial(
    jtu.ignore_warning, category=FutureWarning, message="Using a non-tuple sequence.*"
)
...
class IndexingTest(jtu.TestCase):
    ...
    def testStaticIndexing(self, shape, dtype, rng_factory, indexer):
        ...
        self._CompileAndCheck(
            jnp_fun, args_maker, check_dtypes=True, check_incomplete_shape=True
        )
    ...
    @parameterized.named_parameters(
        {
            "testcase_name": "_{}_inshape={}_indexer={}".format(  # pylint: disable=g-complex-comprehension
                name, jtu.format_shape_dtype_string(shape, dtype), indexer
            ),
            "name": name,
            "shape": shape,
    ...
    def testMixedAdvancedIntegerIndexing(
        self, name, shape, dtype, rng_factory, indexer
    ):
        rng = rng_factory()
    ...
    def testFloatIndexingError(self):
        ...
        # Test with jit
        with self.assertRaisesRegex(IndexError, error_regex):
            npe.jit(lambda idx: jnp.zeros((2, 2))[idx])((0, 0.0))
    ...
    def testDynamicSliceAgainstNumpy(
        self, shape, dtype, start_indices, size_indices, rng_factory
    ):
        rng = rng_factory()
        args_maker = lambda: [rng(shape, dtype), onp.array(start_indices)]
    ...
    def testDynamicSliceInDim(self):
        ...
        x = rng((6, 7), onp.int32)
        self.assertAllClose(
            npe.dynamic_slice_in_dim(x, 2, 3), x[2:5], check_dtypes=True
        )
...
class IndexedUpdateTest(jtu.TestCase):
    ...
    def testStaticIndexing(
        self, shape, dtype, update_shape, update_dtype, rng_factory, indexer, op
    ):
        rng = rng_factory()
        ...
        check_xla = not has_non_trivial_stride(indexer) and not (  # b/123559667
            isinstance(indexer, slice) and indexer.stop == 8 and indexer.step == -1
        )
        self._CompileAndCheck(
            tfnp_fn,
            args_maker,
    ...
    def testStaticIndexingGrads(
        ...
        self.check_grads(tfnp_fn, (x, y), rtol=1e-3, atol=1e-3, delta=1.0)
    ...
    def testDynamicUpdateSlice(
        ...
        #   arguments.
        self._CompileAndCheck(
            npe.dynamic_update_slice, args_maker, check_incomplete_shape=False
        )
    ...
    def testDynamicUpdateSliceAgainstNumpy(
        self, shape, dtype, start_indices, update_shape, rng_factory
    ):
        rng = rng_factory()
        ...
        self._CheckAgainstNumpy(
            dynamic_update_slice_reference, npe.dynamic_update_slice, args_maker
        )
    ...
    def testDynamicUpdateSliceInDim(self):
        ...
        z[2:5] = y
        self.assertAllClose(
            npe.dynamic_update_slice_in_dim(x, y, 2, 0), z, check_dtypes=True
        )
