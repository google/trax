...
"""Tests for trax.reinforcement.advantages."""
...
class AdvantagesTest(parameterized.TestCase):
    ...
    def test_td_k_variance_lower_than_monte_carlo(self):
        (_, var_td_3) = estimate_advantage_bias_and_variance(advantages.td_k, margin=3)
    ...
    def test_td_k_bias_decreases_with_k(self, k1, k2):
        (bias1, _) = estimate_advantage_bias_and_variance(advantages.td_k, margin=k1)
        (bias2, _) = estimate_advantage_bias_and_variance(advantages.td_k, margin=k2)
        self.assertGreater(bias1, bias2)

    @parameterized.named_parameters(("1_2", 1, 2), ("2_3", 2, 3))
    def test_td_k_variance_increases_with_k(self, k1, k2):
        (_, var1) = estimate_advantage_bias_and_variance(advantages.td_k, margin=k1)
        (_, var2) = estimate_advantage_bias_and_variance(advantages.td_k, margin=k2)
    ...
    def test_bias_decreases_with_lambda(self, advantage_fn, lambda1, lambda2):
        (bias1, _) = estimate_advantage_bias_and_variance(advantage_fn, lambda_=lambda1)
        (bias2, _) = estimate_advantage_bias_and_variance(advantage_fn, lambda_=lambda2)
        self.assertGreater(bias1, bias2)
