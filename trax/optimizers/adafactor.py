...
class Adafactor(opt_base.Optimizer):
    ...
    def update(self, step, grads, weights, slots, opt_params):
        ...
        if self._multiply_by_parameter_scale:
            update_scale *= jnp.maximum(jnp.sqrt(jnp.mean(weights * weights)), epsilon2)
        ...
        if self._factored and len(weights.shape) >= 2:
            ...
            v_col = slots[1]
            new_v_row = decay_rate * v_row + mixing_rate * jnp.mean(grads_sqr, axis=-1)
            new_v_col = decay_rate * v_col + mixing_rate * jnp.mean(grads_sqr, axis=-2)
            ...
            y = (
                grads
                * jnp.expand_dims(row_factor, axis=-1)
