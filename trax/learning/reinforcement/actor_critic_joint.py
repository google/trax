...
class ActorCriticJointAgent(rl_training.Agent):
    ...
    def __init__(
        ...
        Args:
          ...
          supervised_evals_per_epoch: number of value trainers evaluations per RL
              epoch - only affects metric reporting.
          supervised_eval_steps: number of value trainers steps per evaluation -
        ...
        self._inputs = data.inputs.Inputs(train_stream=lambda _: self.batches_stream())
...
class PPOJoint(ActorCriticJointAgent):
    ...
    def __init__(
        self, task, epsilon=0.2, value_loss_coeff=0.1, entropy_coeff=0.01, **kwargs
    ):
    ...
    def batches_stream(self):
        ...
        for np_trajectory in self._task.trajectory_batch_stream(
            ...
            yield (
                np_trajectory.observation,  # Inputs to the value model.
    ...
    def joint_loss(self):
        ...
        def f(
            dist_inputs, values, returns, dones, rewards, actions, old_log_probs, mask
        ):
            ...
            assert values.shape == returns.shape, (
                f"values.shape was {values.shape}" f"returns.shape was {returns.shape}"
            )
            assert values.shape == dones.shape, (
                f"values.shape was {values.shape}" f"returns.shape was {dones.shape}"
            )
            ...
            assert len(ppo_objective.shape) == 2, f"ppo_objective was {ppo_objective}"
    ...
    def unclipped_objective_mean(self):
        def f(dist_inputs, values, returns, dones, rewards, actions, old_log_probs):
            ...
            unclipped_objective = rl_layers.UnclippedObjective(probs_ratio, advantages)
...
class A2CJoint(ActorCriticJointAgent):
    ...
    def __init__(self, task, value_loss_coeff=0.1, entropy_coeff=0.01, **kwargs):
    ...
    def joint_loss(self):
        ...
        def f(
            ...
            assert mask.shape[0:2] == returns.shape[0:2], (
                f"mask.shape was {mask.shape}" f"returns.shape was (returns.shape)"
            )
