...
TimeStepBatch = collections.namedtuple(
    "TimeStepBatch",
    [
        "observation",
        "action",
...
# `trax.reinforcement.envs.SequenceDataEnv`. In those environments, one timestep
...
TimeStepBatch.__new__.__defaults__ = (
    EnvInfo(),
    None,
)
...
class Trajectory:
    ...
    def extend(self, new_observation, mask=1, **kwargs):
        ...
        self._timesteps.append(
            TimeStepBatch(observation=self._last_observation, mask=mask, **kwargs)
        )
    ...
    def calculate_returns(self, gamma):
        ...
        discount_mask = np.array([ts.env_info.discount_mask for ts in self._timesteps])
...
def play(env, policy, dm_suite=False, max_steps=None, last_observation=None):
    ...
    while not done and (max_steps is None or cur_step < max_steps):
        ...
        env_info = EnvInfo(
            **{key: value for (key, value) in info.items() if key in EnvInfo._fields}
        )
...
def _sample_proportionally(inputs, weights):
    ...
    if l != len(weights):
        raise ValueError(
            f"Inputs and weights must have the same length, but do not"
            f": {l} != {len(weights)}"
        )
...
class RLTask:
    ...
    def save_to_file(self, file_name):
        ...
        epochs_to_save = [
            e for e in self._trajectories if e not in self._saved_epochs_unchanged
        ]
        for epoch in epochs_to_save:
            training.pickle_to_file(
                self._trajectories[epoch],
    ...
    def collect_trajectories(
        ...
        if n_trajectories is not None:
            new_trajectories = [
                self.play(policy, max_steps=max_steps, only_eval=only_eval)
            ...
            raise ValueError("Either n_trajectories or n_interactions must be defined.")
        ...
        if epoch_id in self._saved_epochs_unchanged:
            self._saved_epochs_unchanged = [
                e for e in self._saved_epochs_unchanged if e != epoch_id
            ]

        # Remove epochs not intended to be in the buffer
        current_trajectories = {
            key: value
            for key, value in self._trajectories.items()
    ...
    def _random_slice(self, trajectory, max_slice_length, margin):
        ...
        slice_start = np.random.randint(_n_slices(trajectory, max_slice_length, margin))
        ...
        return fastmath.nested_map(lambda x: x[slice_start:slice_end], timestep_batch)

    def _trajectory_stream(
        self,
        epochs=None,
        ...
        def new_epoch(epoch_id):
            ...
            epoch_to_total_n_slices[epoch_id] = np.sum(epoch_to_ns_slices[epoch_id])
            ...
            sampling_epoch_weights = np.array(
                list(epoch_to_total_n_slices[ep] for ep in epoch_indices)
            )
    ...
    def trajectory_batch_stream(
        ...
        def pad(tensor_list):
            ...
            return np.array(
                [_zero_pad(t, (0, pad_len - t.shape[0]), axis=0) for t in tensor_list]
            )
