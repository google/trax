...
class Trainer:
    ...
    def __init__(self, model_with_loss, optimizer, n_devices=None, adasum=False):
        ...
        self._forward_and_backward_fn = fastmath.value_and_grad(
            model_with_loss.pure_fn, argnums=1, has_aux=True  # arg1 of pure_fn: weights
...
def _average_multidevice_gradients(gradients, adasum=False):
    ...
    if base.N_WEIGHTS_SHARDS > 1:  # only sum gradients from matching shards
        groups = [
            [base.N_WEIGHTS_SHARDS * i + d for i in range(int(n))]
            for d in range(base.N_WEIGHTS_SHARDS)
        ]
        gradients_psum = fastmath.psum(gradients, "batch", axis_index_groups=groups)
    else:
...
# (weights, slots), step, opt_params, batch, state, rng ->
# (weights, slots), state, stats
def _accelerate_update_fn(
    forward_and_backward_fn, optimizer, n_devices, accelerate=True, adasum=False
):
    ...
    def _multi_device_update_fn(weights_and_slots, step, opt_params, batch, state, rng):
        ...
        weights, slots = weights_and_slots
        (loss, state), gradients = forward_and_backward_fn(batch, weights, state, rng)
    ...
    def multi_device_update_fn(weights_and_slots, step, opt_params, batch, state, rng):
        # Need to replicate step to n_devices leading dimension.
        return _multi_device_update_fn(
            weights_and_slots,
            jnp.repeat(step, n_devices),
            opt_params,
            batch,
            state,
            rng,
...
class ReversibleSerialTrainer:
    ...
    We provide layers to this trainers in blocks, each block consisting of
    ...
    def __init__(
        ...
        This trainers performs updates equivalent to using the default Trainer on::
    ...
    def _per_device_rngs(self, rng):
        ...
        # Splitting by device first to be identical with default trainers.
    ...
    def one_step(self, batch, rng, step=0, learning_rate=None):
        ...
        for i, (std_layer, rev_layers) in enumerate(self._blocks):
            ...
            block_inputs_states.append(
                tl.on_cpu(((std_inputs, std_state), (rev_old_states, rev_new_states)))
            )
...
def _fbo_with_layer_and_opt(layer, optimizer, n_devices, stats_name=None, adasum=False):
    ...
    def fbo(inputs, weights, grads, state, slots, opt_params, rng, step):
        ...
        if n_devices > 1:
            grads_weights = _average_multidevice_gradients(grads_weights, adasum=adasum)
...
def extract_reversible_blocks(layers, loss_chunk_size=0):
    ...
    def _flatten(l):
        ...
        if isinstance(l, (list, tuple)):
            return [
                x for layer in l for x in _flatten(layer)
