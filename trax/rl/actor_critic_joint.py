...
class ActorCriticJointTrainer(rl_training.RLTrainer):
  ...
  def advantage_mean(self):
    """Mean of advantages."""
    return tl.Fn(
        lambda dist_inputs, values, returns, advantages: jnp.mean(advantages),
        n_out=1)
  ...
  def advantage_norm(self):
    """Mean of advantages."""
    return tl.Fn(
        lambda dist_inputs, values, returns, advantages: jnp.linalg.norm(
            advantages),
        n_out=1)
  ...
  def policy_and_value(self, trajectory):
    ...
    pred, value = model(trajectory_np.observations[None, ...], n_accelerators=1)
    # print('pred before {}'.format(pred))
    # print('value before {}'.format(value))
    # Pick element 0 from the batch (the only one), last (current) timestep.
    pred = pred[0, -1, :]
    value = value[0, -1, :]
    # print('pred after {}'.format(pred))
    # print('value after {}'.format(value))
    sample = self._policy_dist.sample(pred)
    log_prob = self._policy_dist.log_prob(pred, sample)
    return (sample.copy(), log_prob.copy(), value.copy())

  def policy(self, trajectory):
    sample, log_prob, _ = self.policy_and_value(trajectory)
    return (sample, log_prob)

  def value_fun(self, trajectory):
    _, _, value = self.policy_and_value(trajectory)
    return value
  ...
  def joint_loss(self):
    ...
    def PPOJointLoss(dist_inputs, values, returns, advantages,
      ...
      ppo_objective = rl_layers.PPOObjective(
          dist_inputs, stop_gradient(values), returns,
