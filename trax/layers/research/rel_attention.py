...
class RelativeAttention(base.Layer):
  ...
  def _fast_inference_update_state(self, inputs, state):
    ...
    ks = fastmath.dynamic_update_slice_in_dim(ks,
                                              new_k,
                                              idx // self._total_kv_pooling,
                                              axis=1)
    vs = fastmath.dynamic_update_slice_in_dim(vs,
                                              new_v,
                                              idx // self._total_kv_pooling,
                                              axis=1)
...
def Sinusoidal_Embeddings(positions, d_feature):
  """
    ...
    Embeddings are shaped (n_positions, d_feature)
  """
  inv_freq = 1 / (10000 ** (jnp.arange(0.0, d_feature, 2.0) /
                            d_feature))
