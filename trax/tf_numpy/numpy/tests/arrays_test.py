...
class ArrayTest(tf.test.TestCase):
  ...
  def testAstype(self):
    a = t2a(tf.convert_to_tensor(1.1, dtype=tf.float32)).astype(np.int32)
    self.assertIs(a.dtype.type, np.int32)
    self.assertAllEqual(1, a)
    a = t2a(tf.convert_to_tensor([0.0, 1.1], dtype=tf.float32)).astype(np.bool_)
    self.assertIs(a.dtype.type, np.bool_)
    self.assertAllEqual([False, True], a)

  def testNeg(self):
    a = t2a(tf.convert_to_tensor([1.0, 2.0]))
    self.assertAllEqual([-1.0, -2.0], -a)

  def _testBinOp(self, a, b, out, f, types=None):
    a = t2a(tf.convert_to_tensor(a, dtype=np.int32))
    b = t2a(tf.convert_to_tensor(b, dtype=np.int32))
    if not isinstance(out, arrays.ndarray):
      out = t2a(tf.convert_to_tensor(out, dtype=np.int32))
    if types is None:
  ...
  def testTruediv(self):
    self._testBinOp([3, 5], [2, 4], t2a(tf.convert_to_tensor([1.5, 1.25])),
  ...
  def testInt(self):
    v = 10
    u = int(t2a(tf.convert_to_tensor(v)))
    self.assertIsInstance(u, int)
    self.assertAllEqual(v, u)

  def testFloat(self):
    v = 21.32
    u = float(t2a(tf.convert_to_tensor(v)))
    self.assertIsInstance(u, float)
    self.assertAllClose(v, u)

  def testBool(self):
    b = bool(t2a(tf.convert_to_tensor(10)))
    self.assertIsInstance(b, bool)
    self.assertTrue(b)
    self.assertFalse(bool(t2a(tf.convert_to_tensor(0))))
    self.assertTrue(bool(t2a(tf.convert_to_tensor(0.1))))
    self.assertFalse(bool(t2a(tf.convert_to_tensor(0.0))))

  def testHash(self):
    a = t2a(tf.convert_to_tensor(10))
