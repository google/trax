    ...
    def forward(self, x):
        ...
        if self._use_bias:
            if not isinstance(self.weights, (tuple, list)):
                raise ValueError(
                    f"Weights should be a (w, b) tuple or list; "
...
class Embedding(base.Layer):
    ...
    def __init__(
        ...
        kernel_initializer=init.ScaledInitializer(
            out_dim=-1, in_dim=-2, scale=1.0, mode="fan_out", distribution="uniform"
        ),
    ):
...
def PrintShape(n_in=1, msg=""):
    ...
    def Fwd(xs):
        def format_shape(x):  # pylint: disable = invalid-name
            return str(jnp.shape(x)) + f"[{x.dtype}]"
...
class SummaryImage(base.Layer):
    ...
    def __init__(self, name, n_in, num_summaries=5, recover_fn=None):
    ...
    def init_weights_and_state(self, input_signature):
        ...
        self.weights = ()
        self.state = {self._name: jnp.array(0.0)}
...
class RandomUniform(base.Layer):
    ...
    def __init__(
        self, min_val=0.0, max_val=1.0, shape=(), dtype=jnp.float32, sync=False
    ):
        """Layer returning a tensor with random values distributed uniformly.
...
class LocallyConnected1d(base.Layer):
    ...
    def init_weights_and_state(self, input_signature):
        ...
        if self._padding == "VALID":
            shape_b = (
                input_signature.shape[-2] - self._kernel_size + 1,
                self._filters,
            )
        else:
            shape_b = (
...
def Flatten(n_axes_to_keep=1):
    ...
    def f(x):  # pylint: disable=invalid-name
        in_rank = len(jnp.shape(x))
        if in_rank <= n_axes_to_keep:
            raise ValueError(
                f"Input rank ({in_rank}) must exceed the number of "
...
def LogSumExp(axis=-1):
    ...
    """
    return Fn("LogSumExp", lambda x: fastmath.logsumexp(x, axis=axis, keepdims=True))
...
def ThresholdToBinary(threshold=0.5):
