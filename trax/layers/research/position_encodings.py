...
class FixedBasePositionalEncoding(layer_base.Layer):
    ...
    def __init__(
        self,
        bases=[11, 13, 14, 15],
        n_digits=8,  #  pylint: disable=dangerous-default-value
        start_from_zero_one_in=2,
        base_dropout_one_in=100,
        mode="train",
        initializer=init.RandomUniformInitializer(1e-4),
    ):
        super().__init__()
        self._bases = bases
...
def threefry_2x32_prf(key, x: jnp.ndarray) -> jnp.ndarray:
    ...
    y = jnp.moveaxis(jnp.reshape(y_3f, (2,) + x.shape[:-1]), source=0, destination=-1)
    return y
...
class InfinitePositionalEncoding(layer_base.Layer):
    ...
    def _get_embeddings(self, lo: int, hi: int, depth, rng=None):
        ...
        freq = jnp.geomspace(0.5, 0.5 * drift**2, num=(depth + 1) // 2)[c_div_2]
