...
class Layer:
    ...
    def __init__(self, n_in=1, n_out=1, name=None, sublayers_to_print=None):
        ...
        self._caller = {
            "filename": copy.copy(frame.f_code.co_filename),
    ...
    def init(self, input_signature, rng=None, use_cache=False):
        ...
        try:
            ...
            raise LayerError(
                name, "init", self._caller, input_signature, trace
            ) from None
    ...
    def init_from_file(self, file_name, weights_only=False, input_signature=None):
        ...
        weights, state = unflatten_weights_and_state(
            dictionary["flat_weights"],
            dictionary["flat_state"],
    ...
    def save_to_file(self, file_name, weights_only=False, input_signature=None):
        ...
        """
        flat_weights, flat_state = flatten_weights_and_state(self.weights, self.state)
        dictionary = {
    ...
    def weights(self):
        ...
        if not self.sublayers:
            ...
            return tuple(
                layer.weights if w is None else w
    ...
    def state(self):
        ...
        if not self.sublayers:
            ...
            return tuple(
                layer.state if s is None else s
    ...
    def pure_fn(self, x, weights, state, rng, use_cache=False):
        ...
        try:
            ...
            name, trace = self._name, _short_traceback(skip=3)
            raise LayerError(
                name, "pure_fn", self._caller, signature(x), trace
            ) from None
...
class LayerError(Exception):
    ...
    def __init__(
        self, layer_name, function_name, caller, input_signature, traceback_string
    ):
...
def flatten_weights_and_state(weights, state):
    ...
    flat_weights = [
        w for w in fastmath.tree_flatten(weights) if not _is_empty_weight(w)
    ]

    def _is_empty_state(x):
        return x is EMPTY_STATE or (isinstance(x, dict) and x == GET_STATE_FROM_CACHE)

    flat_state = [s for s in fastmath.tree_flatten(state) if not _is_empty_state(s)]
...
def unflatten_weights_and_state(
    ...
    weights, _ = fastmath.tree_unflatten(
        flat_weights, weights_tree, copy_from_tree=weights_to_copy
    )
    state = None
    if not weights_only:
        states_to_copy = [EMPTY_STATE, GET_STATE_FROM_CACHE]
        state, _ = fastmath.tree_unflatten(
            flat_state, state_tree, copy_from_tree=states_to_copy
        )
...
def unshard_in_pmap(tensors, n_shards):
    ...
    groups = [
        [n_shards * i + d for d in range(n_shards)]
