...
class ReversibleLayer(base.Layer):
    ...
    def reverse_and_grad(
        self, output, grad, weights=(), state=(), new_state=(), rng=None
    ):
    ...
    def backward(self, inputs, output, grad, weights, state, new_state, rng):
        del inputs
        _, inputs_weights_grad = self.reverse_and_grad(
            output, grad, weights, state, new_state, rng
        )
...
class ReversibleSelect(ReversibleLayer):
    ...
    def __init__(self, indices, n_in=None, name=None):
        ...
        for i in range(n_in):
            if i not in indices:
                raise ValueError(
                    "To be reversible, all inputs to Select must be in "
...
class ReversibleHalfResidual(ReversibleLayer):
    ...
    def reverse_and_grad(
        self, output, ct, weights=(), state=(), new_state=(), rng=None
    ):
        ...
        (
            compute_residual_inputs_ct,
            compute_residual_weights_ct,
        ...
        stack_ct = (
            (accumulator_output_ct,)
            + fastmath.nested_map_multiarg(_add, context_ct[: len(stack_ct)], stack_ct)
