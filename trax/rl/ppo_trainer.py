...
from trax.rl import on_policy_trainer
...
class PPO(on_policy_trainer.OnPolicyTrainer):
  """PPO trainer."""
  ...
  def reset(self, output_dir=None):
    super(PPO, self).reset(output_dir=output_dir)

    if self.epoch > 0:
      # We were already restored from a checkpoint, do nothing.
      return

    # If this wasn't restored from a checkpoint, initialize from a world model
    # if required.
    if self._init_policy_from_world_model_output_dir is not None:
      new_weights = (
          ppo.init_policy_from_world_model_checkpoint(
              self.policy_and_value_net_params,
              self._init_policy_from_world_model_output_dir,
          ))

      # Initialize the optimizer.
      (init_slots, init_opt_params) = (
          self._policy_and_value_optimizer.tree_init(new_weights)
      )
      self._policy_and_value_opt_state = (
          new_weights, init_slots, init_opt_params
      )
