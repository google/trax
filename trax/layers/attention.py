...
def AttentionQKV(
    ...
    return cb.Serial(
        cb.Parallel(
            _SparsifiableDense(q_sparsity), _CacheableDense(), _CacheableDense()
        ),
        _PureAttention(),
...
def _per_head_attention(queries, keys, values, mask, dropout, mode, rng):
    ...
    attn_strengths = jnp.exp(dots - fastmath.logsumexp(dots, axis=-1, keepdims=True))
    if dropout is not None and dropout > 0.0 and mode == "train":
        keep = fastmath.random.bernoulli(rng, 1.0 - dropout, attn_strengths.shape)
        attn_strengths = jnp.where(
            keep, attn_strengths / (1.0 - dropout), jnp.zeros_like(attn_strengths)
        )
...
def SplitIntoHeads(n_heads, merged_batch_and_head=True):
    ...
    def f(x):
        ...
        if merged_batch_and_head:
            x = jnp.reshape(x, (batch_size * n_heads, seq_len, d_head))
        return x
    ...
    def forward(self, inputs):
        ...
        if self._mode == "predict":
            self.state, mask = _fast_inference_update_state(
                inputs, self.state, mask_for_predict=mask_for_predict
            )
            if self._portal_mask is not None:
                (_, k, v, _) = self.state
            else:
    ...
    def init_weights_and_state(self, input_signature):
        ...
        if self._mode == "predict":
            self.state = _fast_inference_init_state(
                input_signature, self._max_len, predict_mask=self._portal_mask
            )
    ...
    def forward(self, inputs):
        ...
        if self._mode != "predict":
            ...
            if self._mode != "train" or self._start_from_zero_prob >= 1.0:
                ...
                start = jnp.where(
                    start_from_zero < self._start_from_zero_prob,
            ...
            if self._dropout == 0:
                ...
                keep = fastmath.random.bernoulli(
                    self.rng, keep_prob, tuple(noise_shape)
                )
...
def _fast_inference_init_state(input_signature, buffer_length, predict_mask=None):
...
def _fast_inference_update_state(inputs, state, mask_for_predict=None):
    ...
    if mask_for_predict is None:
        ...
        state_mask_for_predict = fastmath.dynamic_update_slice_in_dim(
            state_mask_for_predict != 0, mask_for_predict.reshape((-1)) != 0, 0, axis=0
        )
