...
def jax_conv(
    inp, fltr, window_strides, padding, dimension_numbers, filter_dilation=None
):
...
def jax_max_pool(x, pool_size, strides, padding):
    return _pooling_general(
        x, lax.max, -jnp.inf, pool_size=pool_size, strides=strides, padding=padding
    )


def jax_sum_pool(x, pool_size, strides, padding):
    return _pooling_general(
        x, lax.add, 0.0, pool_size=pool_size, strides=strides, padding=padding
    )


def _normalize_by_window_size(
    dims, spatial_strides, padding
):  # pylint: disable=invalid-name
    def rescale(outputs, inputs):
        one = jnp.ones(inputs.shape[1:-1], dtype=inputs.dtype)
        window_sizes = lax.reduce_window(
            one, 0.0, lax.add, dims, spatial_strides, padding
        )
...
def jax_randint(key, shape, minval, maxval, dtype=np.int32):
    ...
    """
    return jax_random.randint(key, shape, minval=minval, maxval=maxval, dtype=dtype)
...
JAX_BACKEND = {
    ...
    "index_add": lambda x, idx, y: jnp.asarray(x).at[idx].add(y),
    "index_max": lambda x, idx, y: jnp.asarray(x).at[idx].max(y),
    "index_min": lambda x, idx, y: jnp.asarray(x).at[idx].min(y),
    "index_update": lambda x, idx, y: jnp.asarray(x).at[idx].set(y),
    "jit": jax.jit,
