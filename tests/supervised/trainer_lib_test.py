...
def _test_inputs(n_classes, with_weights=False, input_shape=(6, 6, 3)):
    ...
    def input_stream(n_devices):
        ...
        while True:
            ...
            key = keys[0]
            inputs = fastmath.random.uniform(keys[1], [batch_size] + list(input_shape))
    ...
    def input_stream(_):
        def make_batch(key):
            return fastmath.random.randint(
                key, [batch_size, seq_len], dtype=jnp.int32, minval=0, maxval=vocab_size
            )
...
class TraxTest(parameterized.TestCase):
    ...
    def _test_train_eval_predict(self, backend, model_name="Simple", optimizer=None):
        with fastmath.use_backend(backend):
            ...
            if model_name == "Simple":
                ...
                model_fn = functools.partial(
                    models.Transformer, input_vocab_size=vocab_size
                )
                n_in = 2
            else:
    ...
    @parameterized.named_parameters(
        ("_%s" % short_name(backend), backend) for backend in BACKENDS
    )
