...
def clip(a, a_min, a_max):  # pylint: disable=missing-docstring
    ...
    if a_min is None:
        ...
        a, a_min, a_max = array_ops._promote_dtype(
            a, a_min, a_max
...
def cross(
    ...
    def f(a, b):  # pylint: disable=missing-docstring
        ...
        def maybe_move_axis_to_last(a, axis):
            def move_axis_to_last(a, axis):
                return tf.transpose(
                    a,
                    tf.concat(
                        [tf.range(axis), tf.range(axis + 1, tf.rank(a)), [axis]], axis=0
                    ),
                )
        ...
        def maybe_pad_0(a, size_of_last_dim):
            ...
            return utils.cond(size_of_last_dim == 2, lambda: pad_0(a), lambda: a)
        ...
        def move_last_to_axis(a, axis):
            r = tf.rank(a)
            return tf.transpose(
                a, tf.concat([tf.range(axis), [r - 1], tf.range(axis, r - 1)], axis=0)
            )
...
def _tf_gcd(x1, x2):
    ...
    def _gcd_body_fn(x1, x2):
        ...
        x1, x2 = (
            tf.where(x2 != 0, x2, x1),
            tf.where(x2 != 0, tf.math.mod(x1, x2_safe), tf.constant(0, x2.dtype)),
        )
    ...
    gcd, _ = tf.while_loop(
        _gcd_cond_fn, _gcd_body_fn, (tf.math.abs(x1), tf.math.abs(x2))
    )
    return gcd
...
def lcm(x1, x2):
    def f(x1, x2):
        ...
        return tf.where(d == 0, tf.constant(0, d.dtype), tf.math.abs(x1 * x2) // d_safe)
...
def _make_nan_reduction(onp_reduction, reduction, init_val):
    ...
    def nan_reduction(a, axis=None, dtype=None, keepdims=False):
        ...
        return reduction(
            array_ops.where(isnan(a), v, a), axis=axis, dtype=dtype, keepdims=keepdims
        )
...
def nanmean(
    ...
    if dtype is None:
        dtype = a.dtype
    normalizer = array_ops.sum(nan_mask, axis=axis, dtype=dtype, keepdims=keepdims)
...
def ptp(a, axis=None, keepdims=None):
    return array_ops.amax(a, axis=axis, keepdims=keepdims) - array_ops.amin(
...
def tile(a, reps):
    ...
    reps = tf.pad(
        reps, [[tf.math.maximum(a_rank - reps_size, 0), 0]], constant_values=1
    )
    a_shape = tf.pad(
        tf.shape(a), [[tf.math.maximum(reps_size - a_rank, 0), 0]], constant_values=1
    )
...
def argsort(
    ...
    tf_ans = tf.cond(
        tf.rank(a) == 0, lambda: tf.constant([0]), lambda: _argsort(a, axis, stable)
    )
...
def average(
    ...
    if weights is None:  # Treat all weights as 1
        ...
        if np.issubdtype(a.dtype, np.inexact):
            ...
            out_dtype = utils.result_type(a.dtype, weights, dtypes.default_float_type())
        ...
        if axis is None:
            ...
            avg, weights_sum = utils.cond(
                tf.rank(a) == tf.rank(weights), rank_equal_case, rank_not_equal_case
            )
