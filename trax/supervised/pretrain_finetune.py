...
def _tfds_stream(
    ...
    if batch_size % n_devices != 0:
        raise ValueError(
            f"Batch size ({batch_size}) not divisible"
            " by number of devices ({n_devices})"
        )
    ds = tfds.load(
        name=dataset_name, split=split, data_dir=data_dir, shuffle_files=shuffle_files
    )
    if repeat:
        ds = ds.repeat()
...
def bert_preprocess(batch, tokenizer, key_a, key_b=None, max_len=128):
    ...
    for i in range(batch_size):
        ...
        tokens_a = (
            [101]
            + tokenizer.convert_tokens_to_ids(tokenizer.tokenize(sentence_a))
            + [102]
        )
...
def glue_inputs(
    ...
    preprocess_fn = functools.partial(
        bert_preprocess, tokenizer=tokenizer, key_a=key_a, key_b=key_b, max_len=max_len
    )
