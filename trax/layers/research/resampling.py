...
def LinearPooling(shorten_factor, d_model, *args, dropout=0.0, mode="train", **kwargs):
    del args, kwargs
    ...
    return cb.Serial(
        core.Fn(
            ...
            lambda x: jnp.reshape(  # pylint: disable=g-long-lambda
                ...
                x,
                (x.shape[0], x.shape[1] // shorten_factor, -1),
            ),
            n_out=1,
        ),
        core.Dense(d_model),
...
def LinearUpsampling(
    shorten_factor, d_model, *args, dropout=0.0, mode="train", **kwargs
):
    del args, kwargs
...
def NaiveUpsampling(
    shorten_factor, d_model, *args, **kwargs
