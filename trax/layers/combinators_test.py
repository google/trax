...
class SerialTest(absltest.TestCase):
    def test_none_is_no_op(self):
        ...
        xs = [np.array([1, 2, 3, 4]), np.array([10, 20, 30])]
        ys = layer(xs)
        self.assertEqual(as_list(ys), [[1, 2, 3, 4], [10, 20, 30]])
    ...
    def test_one_in_two_out(self):
        layer = tl.Serial(DivideBy(3), ReturnConst(np.array([3, 4, 5, 6])))
        x = np.array([3, 6, 9, 12])
        y = layer(x)
        self.assertEqual(as_list(y), [[3, 4, 5, 6], [1, 2, 3, 4]])
    ...
    def test_flat_weights_and_state(self):
        ...
        weights2, state2 = tl.unflatten_weights_and_state(flat_weights, flat_state, sig)
...
class ParallelTest(absltest.TestCase):
    def test_dup_dup(self):
        ...
        ys = layer(xs)
        self.assertEqual(as_list(ys), [[1, 2, 3], [1, 2, 3], [10, 20], [10, 20]])
    ...
    def test_two_no_ops(self):
        layer = tl.Parallel(tl.Select([0]), tl.Select([0]))
        xs = (np.array([1, 2, 3]), np.array([10, 20]))
        ys = layer(xs)
...
class ConcatenateTest(absltest.TestCase):
    ...
    def test_with_defaults(self):
        ...
        xs = [np.array([[1, 2, 3], [4, 5, 6]]), np.array([[10, 20, 30], [40, 50, 60]])]
        ys = layer(xs)
        self.assertEqual(as_list(ys), [[1, 2, 3, 10, 20, 30], [4, 5, 6, 40, 50, 60]])

    def test_axis_0(self):
        layer = tl.Concatenate(axis=0)
        xs = [np.array([[1, 2, 3], [4, 5, 6]]), np.array([[10, 20, 30], [40, 50, 60]])]
        y = layer(xs)
        self.assertEqual(as_list(y), [[1, 2, 3], [4, 5, 6], [10, 20, 30], [40, 50, 60]])

    def test_axis_1(self):
        layer = tl.Concatenate(axis=1)
        xs = [np.array([[1, 2, 3], [4, 5, 6]]), np.array([[10, 20, 30], [40, 50, 60]])]
        y = layer(xs)
        self.assertEqual(as_list(y), [[1, 2, 3, 10, 20, 30], [4, 5, 6, 40, 50, 60]])
...
class BranchTest(absltest.TestCase):
    def test_noop_dup(self):
        layer = tl.Branch(tl.Select([0]), tl.Dup())
        x = np.array([1, 2, 3])
        ys = layer(x)
        self.assertEqual(as_list(ys), [[1, 2, 3], [1, 2, 3], [1, 2, 3]])
...
class SwapTest(absltest.TestCase):
    def test_swap(self):
        ...
        xs = [np.array([1, 2, 3]), np.array([10, 20, 30])]
        ys = layer(xs)
        self.assertEqual(as_list(ys), [[10, 20, 30], [1, 2, 3]])
...
class SerialWithSideOutputsTest(absltest.TestCase):
    def test_serial_with_side_outputs_div_div(self):
        ...
        xs = (np.array([1, 2, 3]), np.array([10, 20, 30, 40, 50]), np.array([100, 200]))
        ys = layer(xs)
...
class ScanTest(parameterized.TestCase):
    ...
    def test_multi_input(self, backend):
        ...
        with fastmath.use_backend(backend):
            ...
            self.assertEqual(
                as_list(ys),
                [[[4, 6, 8], [40, 60, 80]], [[4, 5, 6], [40, 50, 60]], [9003, 8003]],
            )
...
class CondTest(absltest.TestCase):
    ...
    def test_complex_blocks(self):
        ...
        ys = layer(xs)
        self.assertEqual(as_list(ys), [0.0, 0.5, 1.0, 1.5, 2.0])
    ...
    def test_condition_func_true(self):
        ...
        false = DivideBy(4.0)
        layer = tl.Cond(cond, true, false)
        xs = (np.array(2.0), np.array([4.0, 12.0]))
        layer.init(shapes.signature(xs))
        ys = layer(xs)
        self.assertEqual(as_list(ys), [2.0, 6.0])
    ...
    def test_condition_func_false(self):
        ...
        false = DivideBy(4.0)
        layer = tl.Cond(cond, true, false)
        xs = (np.array(4.0), np.array([4.0, 12.0]))
        layer.init(shapes.signature(xs))
        ys = layer(xs)
        self.assertEqual(as_list(ys), [1.0, 3.0])
    ...
    def test_condition_func_default_false(self):
        ...
        layer.init(shapes.signature(xs))
        ys = layer(xs)
        self.assertEqual(as_list(ys), [4.0, 12.0])
    ...
    def test_exception_run1(self):
        ...
        for xs in ((), (np.array(4.0), np.array([4.0, 12.0]))):
            layer = tl.Cond(cond, true, false)
    ...
    def test_weights_and_state(self):
        ...
        layer = tl.Cond(cond, true, false)
        xs = (np.array(2.0), np.array([0.0, 1.0, 2.0]))
        layer.init(shapes.signature(xs))

        # weights
        self.assertEqual(
            as_list(layer.weights), as_list((cond.weights, true.weights, false.weights))
        )
        ...
        self.assertEqual(
            as_list(layer.weights), as_list((cond.weights, true.weights, true.weights))
        )
