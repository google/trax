...
class DecodingTest(test.TestCase):
    def test_autoregressive_sample_transformerlm(self):
        model = models.TransformerLM(
            10, d_model=32, d_ff=64, n_layers=1, n_heads=2, mode="predict"
        )
        ...
        s2 = decoding.autoregressive_sample(model, batch_size=2, max_length=10)
        self.assertEqual(s2.shape[0], 2)
        self.assertLess(s2.shape[1], 11)
        model.init(shapes.ShapeDtype((1, 1), dtype=np.int32))
        prefix = np.array([[1, 2, 3]])
        s3 = decoding.autoregressive_sample(
            model, prefix, eos_id=-1, max_length=10, batch_size=1
        )
    ...
    def test_autoregressive_sample_transformer(self):
        ...
        model.init(
            (shapes.signature(inputs), shapes.ShapeDtype((1, 1), dtype=np.int32))
        )
        s = decoding.autoregressive_sample(
            model, inputs=inputs, eos_id=-1, max_length=10
        )
    ...
    def test_autoregressive_sample_transformerlm_quality(self):
        ...
        s = decoding.autoregressive_sample(
            pred_model, inputs, max_length=6, temperature=0.0
        )
    ...
    def test_autoregressive_sample_transformerlm_quality_eval(self):
        ...
        inputs = np.array([[0, 3, 7, 5, 3, 2, 4, 0]], dtype=np.int32)
        s = decoding.autoregressive_sample(
            eval_model, inputs, eval_mode=True, max_length=6, temperature=0.0
        )
        self.assertEqual(str(s[0]), "[3 7 5 3 2 4]")
    ...
    def test_autoregressive_sample_terraformer_lsh(self):
        ...
        # 0w0w
        inputs = np.array(
            [[0, 3, 7, 5, 3, 2, 4, 1, 8, 0, 3, 7, 5, 3, 2, 4, 1, 8]], dtype=np.int32
        )
        inputs = np.pad(
            inputs,
            [(0, 0), (0, max_len - inputs.shape[1])],
            mode="constant",
            constant_values=0,
        )
        s = decoding.autoregressive_sample(
        ...
        for test_len in test_lengths:
            ...
            for _ in range(2):  # Set low to make the test run reasonably fast.
                ...
                inputs = np.random.randint(
                    low=1, high=vocab_size - 1, size=(1, max_len)
                )
    ...
    def test_autoregressive_sample_reformerlm_lsh(self):
        ...
        pred_model = models.ReformerLM(
            ...
            d_attention_key=64,
            d_attention_value=64,
            attention_type=functools.partial(
                tl.LSHSelfAttention,
                chunk_len=16,
                n_hashes=2,
                n_buckets=[32, 32],
    ...
    def test_autoregressive_sample_terraformer_pure_lsh(self):
        ...
        pred_model = models.ConfigurableTerraformer(
            ...
            d_attention_key=64,
            d_attention_value=64,
            encoder_attention_type=self._pure_lsh_self_attention_fn(n_chunks_after=1),
