...
def _f_score(precision, recall, beta2):  # pylint: disable=invalid-name
    ...
    """
    return _non_nan((beta2 + 1) * (precision * recall) / ((beta2 * precision) + recall))
...
def MaskedSequenceAccuracy():
    ...
    def f(model_output, targets, weights):  # pylint: disable=invalid-name
        ...
        position_is_accurate = jnp.logical_or(
            jnp.equal(predictions, targets), position_is_padding
        )
...
def Accuracy(classifier=core.ArgMax()):
    ...
    """
    return cb.Serial(
        classifier, _Accuracy(), _WeightedMean(), name="Accuracy", sublayers_to_print=[]
    )
...
def CrossEntropyLoss():
    ...
    """
    return cb.Serial(
        _CrossEntropy(), _WeightedMean(), name="CrossEntropyLoss", sublayers_to_print=[]
    )
...
def MacroAveragedFScore(beta=1.0, initial_category_index=0):
...
def WeightedFScore(beta=1.0, initial_category_index=0):
...
def CrossEntropySum():
    ...
    return cb.Serial(
        _CrossEntropy(), WeightedSum(), name="CrossEntropySum", sublayers_to_print=[]
    )
...
def _category_cross_entropy(  # pylint: disable=invalid-name
    ...
    if label_smoothing:
        ...
        target_distributions *= 1.0 - label_smoothing
