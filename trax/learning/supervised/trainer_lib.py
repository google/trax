...
class Trainer:
    ...
    A trainers allows to make training steps, train for full epochs,
    ...
    def __init__(
        ...
        if fastmath.is_backend(fastmath.Backend.JAX):
            ...
            new_opt_state_and_model_state = fastmath.jit(new_opt_state_and_model_state)
        self._new_opt_state_and_model_state = lambda: new_opt_state_and_model_state(
            init_rng
        )
    ...
    def reset(self, output_dir, init_checkpoint=None):
        ...
        if self._should_write_summaries:
            self._train_sw = jaxboard.SummaryWriter(
                os.path.join(output_dir, "train"), enable=self._is_chief
            )
            self._eval_sw = jaxboard.SummaryWriter(
                os.path.join(output_dir, "eval"), enable=self._is_chief
            )

        # Reset the train and eval streams.
        self._train_stream = _repeat_stream(self._inputs.train_stream, self._n_devices)
        ...
        self._eval_stream = _repeat_stream(self._inputs.eval_stream, self._n_devices)
        ...
        if output_dir is not None:
            state = load_trainer_state(
                output_dir, self._model_with_loss, init_checkpoint
            )
        else:
            state = TrainerState(
                step=None,
    ...
    def train_epoch(self, n_steps, n_eval_steps):
        ...
        if self._train_sw and n_steps > 1:
            self._train_sw.scalar(
                "training/steps per second", n_steps / elapsed_time, step=self._step
            )
    ...
    def evaluate(self, n_eval_steps):
        ...
        train_metrics, _ = self.evaluation_round(train_eval_slice, weights, state, rng)
        ...
        self._history.append(
            "train", "training/learning_rate", self._step, self.learning_rate
        )
    ...
    def evaluation_round(self, inputs_stream, weights, state, rng):
        ...
        Returns:
          ...
          state returned by this trainers's `predict_fn`.
        """
    ...
    def save_gin(self):
        ...
        sw = self._train_sw
        if sw:
            sw.text("gin_config", jaxboard.markdownify_operative_config_str(config_str))
    ...
    def save_state(self, keep, prefix="model"):
        """Save trainers state given a possibly replicated opt_state."""
        opt_state = self._opt_state
        if self.n_devices > 1:
        ...
        if keep:
            weights_file = os.path.join(output_dir, "{}_{}.pkl.gz".format(prefix, step))
            self._save_state_dict(trainer_state_dict, weights_file)
    ...
    def save_computation_graphs(self):
        ...
        weights = self._opt_state.weights[0]
        forward_computation = (
            jax.jit(self._model_predict_eval)
            .lower(
                batch, weights=weights, state=self._model_state[0], rng=self._rngs[0]
            )
    ...
    def log_metrics(self, metrics, summ_writer, log_prefix):
        ...
        for name, value in metrics.items():
            self.log_step(
                "%s %s | % .8f" % (log_prefix.ljust(5), name.rjust(rjust_len), value)
            )
    ...
    def _should_log_now(self):
        return self._train_sw is not None and (self._step == 1 or self._step % 10 == 0)
...
def _jit_update_fn(predict_fn, loss_fn, optimizer, n_devices, jit=True):
    ...
    def update(weights_and_slots, i, opt_params, batch, state, rng):
        return mapped_update(
            weights_and_slots, np.repeat(i, n_devices), opt_params, batch, state, rng
        )
...
def make_trainer_state_dict(step, opt_state, history, model_state, input_signature):
    """Creates a trainers state dictionary to save to disk.
...
def trainer_state_from_dict(trainer_state_dict, model):
    """Given the trainers state dictionary, returns `TrainerState`."""
    ...
    weights, model_state = tl.unflatten_weights_and_state(
        trainer_state_dict["flat_weights"],
        trainer_state_dict["flat_state"],
