...
def generate_params_inputs_targets(num_examples=1000):
    params = (tf_np.asarray(tf.constant(5.0)), tf_np.asarray(tf.constant(0.0)))

    params_true = (tf_np.asarray(tf.constant(3.0)), tf_np.asarray(tf.constant(2.0)))
...
class ExtensionsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def testCustomGrad(self):
        ...
        x, dy = tf.nest.map_structure(
            lambda shape: uniform(rng, shape, dtype), [x_shape, y_shape]
        )
        expected_y = fwd(*x)
    ...
    @parameterized.named_parameters(
        [
            (  # pylint: disable=g-complex-comprehension
                ("_%s_%s_%s" % (decorator_id, x_struct, y_struct))
                .replace(" ", "")
                .replace("None", ""),
                decorator,
                x_struct,
    ...
    def testVjp(self, has_aux):
        ...
        rng = tf.random.Generator.from_seed(1234)
        x, dy_list = tf.nest.map_structure(
            lambda shape: uniform(rng, shape, dtype), [x_shape, [y_shape] * 2]
        )
        tf_x = x
        ...
        self.assertAllClose(expected_y, y)
        for dy in dy_list:
            expected_dx = tape.gradient(expected_y, tf_x, output_gradients=dy)
    ...
    def testEvalOnShapesNested(self):
        transformer = functools.partial(
            extensions.eval_on_shapes, allow_static_outputs=True
        )
    ...
    def testEvalOnShapesNoUnnecessaryTracing(self):
        def num_traces(f):
            return len(f._tf_function._list_all_concrete_functions_for_serialization())
    ...
    @parameterized.named_parameters(
        [
            ...
            for padding in ["SAME"]
            for lhs_dilation, rhs_dilation in [(None, (1, 1))]
            for dimension_numbers, perms in [
                (("NHWC", "HWIO", "NHWC"), ([0, 2, 3, 1], [2, 3, 1, 0]))
            ]
        ]
    )
    def testConvGeneralDilated(
        self,
        lhs_shape,
        rhs_shape,
        strides,
        ...
        tf_conv = extensions.tf_conv_general_dilated(
            lhs,
            rhs,
            strides,
            padding,
    ...
    def testConv(self):
        y = extensions.conv(
            ...
            np.ones([3, 4, 3, 11], dtype=np.float32),
            [1, 1],
            "SAME",
            ("NHWC", "HWIO", "NHWC"),
        )
    ...
    def assertDTypesEqual(self, a, b):
        ...
        self.assertEqual(
            tf.nest.map_structure(get_dtype, a), tf.nest.map_structure(get_dtype, b)
        )

    @parameterized.named_parameters(
        (
            f"_{jit_scan}_{jit_f}",
            jit_scan,
            jit_f,
        )  # pylint: disable=g-complex-comprehension
    ...
    def testScanImpl(self, jit_scan, jit_f):
        ...
        def f(c, a):
            ...
            assert c.shape == (4,)
            b = tf_np.cos(
                tf_np.sum(tf_np.sin(a))
                + tf_np.sum(tf_np.cos(c))
        ...
        if jit_scan == "no_xla":
            ...
            scan = extensions.jit(
                extensions.scan, static_argnums=(0,), xla_forced_compile=True
            )
        else:
    ...
    @parameterized.named_parameters(
        (
            f"_{jit_grad}_{jit_scan}_{jit_f}",
            jit_grad,
            jit_scan,
            jit_f,
        )  # pylint: disable=g-complex-comprehension
    ...
    def testScanGrad(self, jit_grad, jit_scan, jit_f):
        ...
        if jit_scan == "no_xla":
            ...
            scan = extensions.jit(
                extensions.scan, static_argnums=(0,), experimental_compile=True
            )
        else:
            scan = extensions.scan
    ...
    def testScanShape(self, f, inputs, expected_outputs):
        outputs = extensions.eval_on_shapes(
            functools.partial(extensions.scan, f), static_argnums=(2,)
        )(*inputs)
    ...
    def testDatasetAsNumpy(self):
        arrs = extensions.dataset_as_numpy([tf.constant([1, 2]), tf.constant([3, 4])])
        for a in arrs:
    ...
    def testPmapSimpleModel(self):
        ...
        def _train_and_reduce(params, inputs, targets, learning_rate=0.1):
            ...
            return (
                extensions.psum(new_w) / n_devices,
    ...
    def testPsumStruct(self):
        ...
        def reduce_sum(a):
            a = extensions.psum(a)
            tf.nest.map_structure(lambda x: self.assertIsInstance(x, tf_np.ndarray), a)
            return a
    ...
    def testRematJitXla(self):
        ...
        actual = extensions.jit(extensions.grad(f_remat), xla_forced_compile=True)(a, b)
        expected = extensions.jit(extensions.grad(f), xla_forced_compile=True)(a, b)
        self.assertAllClose(actual, expected)

        actual = extensions.jit(extensions.grad(f_remat), experimental_compile=True)(
            a, b
        )
        expected = extensions.jit(extensions.grad(f), experimental_compile=True)(a, b)
        self.assertAllClose(actual, expected)

    def testStaticStopGradient(self):
        self.assertEqual(extensions.stop_gradient(5.0), 5.0)
        self.assertEqual(type(extensions.stop_gradient(5.0)), type(5.0))

        self.assertEqual(extensions.stop_gradient(tf_np.asarray(5.0)), 5.0)
        self.assertNotEqual(
            type(extensions.stop_gradient(tf_np.asarray(5.0))), type(5.0)
        )
